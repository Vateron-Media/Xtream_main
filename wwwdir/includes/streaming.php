                ...
                if (!(ipTV_lib::$settings['split_by'] == 'maxclients')) {
                    if (ipTV_lib::$settings['split_by'] == 'guar_band') {
                        foreach ($rows as $server_id => $row) {
                            $rows[$server_id]['capacity'] = (float) ($row['online_clients'] / ipTV_lib::$StreamingServers[$server_id]['network_guaranteed_speed']);
                        }
                    } else {
                        foreach ($rows as $server_id => $row) {
                        ...
                        foreach ($rows as $server_id => $row) {
                            $rows[$server_id]['capacity'] = (float) ($row['online_clients'] / $D8d3ca7afab93e5c110124dc7611906c[$server_id]);
                        }
                        foreach ($rows as $server_id => $row) {
                            $rows[$server_id]['capacity'] = (float) ($row['online_clients'] / ipTV_lib::$StreamingServers[$server_id]['total_clients']);
                        }
                    }
