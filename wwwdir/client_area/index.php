...
if (!(!empty($_SESSION["client_loggedin"]) && $_SESSION["client_loggedin"] === true && !empty($_SESSION["cl_data"]))) {
    ...
    $ipTV_db->query("SELECT * FROM `lines` WHERE `username` = ? AND `password` = ? AND (`exp_date` >= " . time() . " OR `exp_date` is null) LIMIT 1", ipTV_lib::$request["username"], ipTV_lib::$request["password"]);
