...
if ($rExtension) {
    ...
    if (!($streamID && (file_exists(STREAMS_TMP_PATH . 'stream_' . $streamID)))) {
        generateError('INVALID_STREAM_ID');
    }

    if ((!ipTV_lib::$settings['show_not_on_air_video'] && file_exists(CACHE_TMP_PATH . 'servers'))) {
    ...
    if (isset(ipTV_lib::$request['token'])) {
        ...
        if (isset(ipTV_lib::$request['hmac'])) {
            if (!in_array($rType, array('live', 'movie', 'series'))) {
                $rDeny = false;
                generateError('INVALID_TYPE_TOKEN');
            }

            $rIdentifier = (empty(ipTV_lib::$request['identifier']) ? '' : ipTV_lib::$request['identifier']);
            $rHMACIP = (empty(ipTV_lib::$request['ip']) ? '' : ipTV_lib::$request['ip']);
            $rMaxConnections = (isset(ipTV_lib::$request['max']) ? intval(ipTV_lib::$request['max']) : 0);
            $rExpiry = (isset(ipTV_lib::$request['expiry']) ? ipTV_lib::$request['expiry'] : null);
            ...
            $rIsHMAC = ipTV_streaming::validateHMAC(ipTV_lib::$request['hmac'], $rExpiry, $streamID, $rExtension, $userIP, $rHMACIP, $rIdentifier, $rMaxConnections);
                ...
                switch ($rExtension) {
                    case 'm3u8':
                        ...
                        if (!$rIsHMAC && is_array($rAdaptive) && 0 < count($rAdaptive)) {
                            ...
                            if (!$rIsHMAC) {
                                ...
                                $tokenData = array('stream_id' => $streamID, 'hmac_hash' => ipTV_lib::$request['hmac'], 'hmac_id' => $rIsHMAC, 'identifier' => $rIdentifier, 'extension' => $rExtension, 'channel_info' => array('redirect_id' => $rChannelInfo['redirect_id'], 'pid' => $rChannelInfo['pid'], 'on_demand' => $rChannelInfo['on_demand'], 'llod' => $rChannelInfo['llod'], 'monitor_pid' => $rChannelInfo['monitor_pid']), 'user_info' => $rUserInfo, 'pid' => $PID, 'external_device' => $rExternalDevice, 'activity_start' => $rActivityStart, 'country_code' => $rCountryCode, 'video_codec' => $rVideoCodec, 'uuid' => $rUUID);
                            }
                    ...
                    case 'ts':
                        ...
                        if (!$rIsHMAC) {
                            ...
                            $tokenData = array('stream_id' => $streamID, 'hmac_hash' => ipTV_lib::$request['hmac'], 'hmac_id' => $rIsHMAC, 'identifier' => $rIdentifier, 'extension' => $rExtension, 'channel_info' => array('stream_id' => $rChannelInfo['stream_id'], 'redirect_id' => ($rChannelInfo['redirect_id'] ?: null), 'pid' => $rChannelInfo['pid'], 'on_demand' => $rChannelInfo['on_demand'], 'llod' => $rChannelInfo['llod'], 'monitor_pid' => $rChannelInfo['monitor_pid'], 'proxy' => $rChannelInfo['direct_proxy']), 'user_info' => $rUserInfo, 'pid' => $PID, 'prebuffer' => $rPrebuffer, 'country_code' => $rCountryCode, 'activity_start' => $rActivityStart, 'external_device' => $rExternalDevice, 'video_codec' => $rVideoCodec, 'uuid' => $rUUID);
                        }
