...
extern void php_mysqli_close(MY_MYSQL *mysql, int close_type,
                             int resource_status);

extern const zend_object_iterator_funcs php_mysqli_result_iterator_funcs;
extern zend_object_iterator *
php_mysqli_result_get_iterator(zend_class_entry *ce, zval *object, int by_ref);

extern void php_mysqli_fetch_into_hash_aux(zval *return_value,
                                           MYSQL_RES *result,
                                           zend_long fetchtype);
...
#define MYSQLI_REGISTER_RESOURCE(__ptr, __ce)                                  \
  {                                                                            \
    zval *object = getThis();                                                  \
    if (!object ||                                                             \
        !instanceof_function(Z_OBJCE_P(object), mysqli_link_class_entry)) {    \
      object = return_value;                                                   \
...
#define MYSQLI_FETCH_RESOURCE_CONN(__ptr, __id, __check)                       \
  {                                                                            \
    MYSQLI_FETCH_RESOURCE((__ptr), MY_MYSQL *, (__id), "mysqli_link",          \
                          (__check));                                          \
    if (!(__ptr)->mysql) {                                                     \
      mysqli_object *intern = Z_MYSQLI_P(__id);                                \
      php_error_docref(NULL, E_WARNING, "invalid object or resource %s\n",     \
...
#define MYSQLI_FETCH_RESOURCE_STMT(__ptr, __id, __check)                       \
  {                                                                            \
    MYSQLI_FETCH_RESOURCE((__ptr), MY_STMT *, (__id), "mysqli_stmt",           \
                          (__check));                                          \
    if (!(__ptr)->stmt) {                                                      \
      mysqli_object *intern = Z_MYSQLI_P(__id);                                \
      php_error_docref(NULL, E_WARNING, "invalid object or resource %s\n",     \
