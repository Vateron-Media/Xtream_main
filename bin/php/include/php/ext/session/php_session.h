...
#define PS_READ_ARGS                                                           \
  void **mod_data, zend_string *key, zend_string **val, zend_long maxlifetime
...
#define PS_UPDATE_TIMESTAMP_FUNC(x)                                            \
  int ps_update_timestamp_##x(PS_UPDATE_TIMESTAMP_ARGS)
...
#define PS_MOD(x)                                                              \
  #x, ps_open_##x, ps_close_##x, ps_read_##x, ps_write_##x, ps_delete_##x,     \
      ps_gc_##x, php_session_create_id, php_session_validate_sid,              \
...
#define PS_MOD_SID(x)                                                          \
  #x, ps_open_##x, ps_close_##x, ps_read_##x, ps_write_##x, ps_delete_##x,     \
      ps_gc_##x, ps_create_sid_##x, php_session_validate_sid,                  \
      php_session_update_timestamp
...
#define PS_MOD_UPDATE_TIMESTAMP(x)                                             \
  #x, ps_open_##x, ps_close_##x, ps_read_##x, ps_write_##x, ps_delete_##x,     \
      ps_gc_##x, ps_create_sid_##x, ps_validate_sid_##x,                       \
      ps_update_timestamp_##x
...
typedef struct _php_ps_globals {
  ...
  zend_bool
      use_trans_sid; /* contains the INI value of whether to use trans-sid */
  ...
  zend_bool
      use_strict_mode;  /* whether or not PHP accepts unknown session ids */
...
PHPAPI zval *php_set_session_var(zend_string *name, zval *state_val,
...
PHPAPI int php_session_register_serializer(
    const char *name, zend_string *(*encode)(PS_SERIALIZER_ENCODE_ARGS),
    int (*decode)(PS_SERIALIZER_DECODE_ARGS));
