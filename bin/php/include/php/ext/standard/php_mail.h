...
PHPAPI extern int php_mail(char *to, char *subject, char *message,
                           char *headers, char *extra_cmd);
    ...
    if (Z_TYPE_P(val) == IS_STRING) {                                          \
      ...
      if (!strncasecmp(target, ZSTR_VAL(key), ZSTR_LEN(key))) {                \
        php_error_docref(NULL, E_WARNING,                                      \
                         "'%s' header must be at most one header. Array is "   \
                         "passed for '%s'",                                    \
                         target, target);                                      \
        continue;                                                              \
      }                                                                        \
      ...
      php_error_docref(                                                        \
          NULL, E_WARNING,                                                     \
          "Extra header element '%s' cannot be other than string or array.",   \
          ZSTR_VAL(key));                                                      \
    }                                                                          \
  } while (0)
