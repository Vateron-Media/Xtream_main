...
enum pdo_fetch_type {
  ...
  PDO_FETCH_CLASS,    /* create an instance of named class, call ctor and set
                         properties */
  ...
  PDO_FETCH_KEY_PAIR, /* fetch into an array where the 1st column is a key and
...
#define PDO_FETCH_UNIQUE                                                       \
  0x00030000 /* fetch into groups assuming first col is unique */
#define PDO_FETCH_CLASSTYPE                                                    \
  0x00040000 /* fetch class gets its class name from 1st column */
#define PDO_FETCH_SERIALIZE                                                    \
  0x00080000 /* fetch class instances by calling serialize */
...
enum pdo_attribute_type {
  ...
  PDO_ATTR_STATEMENT_CLASS, /* array(classname, array(ctor_args)) to specify the
                               class of the constructed statement */
  PDO_ATTR_FETCH_TABLE_NAMES, /* include table names in the column names, where
                                 available */
  PDO_ATTR_FETCH_CATALOG_NAMES, /* include the catalog/db name names in the
                                   column names, where available */
  PDO_ATTR_DRIVER_NAME, /* name of the driver (as used in the constructor) */
  PDO_ATTR_STRINGIFY_FETCHES,  /* converts integer/float types to strings during
                                  fetch */
  ...
  PDO_ATTR_DEFAULT_STR_PARAM,  /* set the default string parameter type (see the
                                  PDO::PARAM_STR_* magic flags) */
...
static inline zend_long pdo_attr_lval(zval *options,
...
static inline zend_string *pdo_attr_strval(zval *options,
                                           enum pdo_attribute_type option_name,
                                           zend_string *defval) {
  zval *v;
...
#define PDO_DRIVER_HEADER(name) #name, sizeof(#name) - 1, PDO_DRIVER_API
...
/* return last insert id.  NULL indicates error condition, otherwise, the return
 * value MUST be an emalloc'd NULL terminated string. */
 ...
 *   specific data ...  */
typedef int (*pdo_dbh_fetch_error_func)(pdo_dbh_t *dbh, pdo_stmt_t *stmt,
                                        zval *info);
...
typedef const zend_function_entry *(*pdo_dbh_get_driver_methods_func)(
    pdo_dbh_t *dbh, int kind);
...
typedef int (*pdo_stmt_param_hook_func)(pdo_stmt_t *stmt,
                                        struct pdo_bound_param_data *param,
                                        enum pdo_param_event event_type);

/* setting of attributes */
typedef int (*pdo_stmt_set_attr_func)(pdo_stmt_t *stmt, zend_long attr,
                                      zval *val);

/* fetching of attributes */
typedef int (*pdo_stmt_get_attr_func)(pdo_stmt_t *stmt, zend_long attr,
                                      zval *val);

/* retrieves meta data for a numbered column.
 * Returns SUCCESS/FAILURE.
 * On SUCCESS, fill in return_value with an array with the following fields.
 * If a particular field is not supported, then the driver simply does not add
 * it to the array, so that scripts can use isset() to check for it.
 *
 ...
 * type name native_type => a string representation of the type, native style,
 * if different from the mapped name. flags => an array of flags including zero
 * or more of the following: primary_key, not_null, unique_key, multiple_key,
 * unsigned, auto_increment, blob
 *
 ...
 * existing key
 */
typedef int (*pdo_stmt_get_column_meta_func)(pdo_stmt_t *stmt, zend_long colno,
                                             zval *return_value);
...
static inline pdo_dbh_t *php_pdo_dbh_fetch_inner(zend_object *obj) {
  return (pdo_dbh_t *)(((pdo_dbh_object_t *)((char *)(obj)-XtOffsetOf(
                            pdo_dbh_object_t, std)))
                           ->inner);
}
...
struct pdo_bound_param_data {
  ...
  zend_long paramno; /* if -1, then it has a name, and we don't know the index
                        *yet* */
  zend_string *name;
...
PDO_API int pdo_parse_params(pdo_stmt_t *stmt, char *inquery,
                             size_t inquery_len, char **outquery,
                             size_t *outquery_len);
