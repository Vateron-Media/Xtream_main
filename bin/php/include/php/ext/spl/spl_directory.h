...
spl_filesystem_from_obj(zend_object *obj) /* {{{ */
{
  return (spl_filesystem_object *)((char *)(obj)-XtOffsetOf(
      spl_filesystem_object, std));
}
/* }}} */
...
#define SPL_FILE_DIR_CURRENT_AS_FILEINFO                                       \
  0x00000000 /* make RecursiveDirectoryTree::current() return SplFileInfo */
#define SPL_FILE_DIR_CURRENT_AS_SELF                                           \
  0x00000010 /* make RecursiveDirectoryTree::current() return getSelf() */
#define SPL_FILE_DIR_CURRENT_AS_PATHNAME                                       \
  0x00000020 /* make RecursiveDirectoryTree::current() return getPathname() */
#define SPL_FILE_DIR_CURRENT_MODE_MASK                                         \
  0x000000F0 /* mask RecursiveDirectoryTree::current() */
...
#define SPL_FILE_DIR_KEY_AS_PATHNAME                                           \
  0x00000000 /* make RecursiveDirectoryTree::key() return getPathname() */
...
#define SPL_FILE_DIR_FOLLOW_SYMLINKS                                           \
  0x00000200 /* make RecursiveDirectoryTree::hasChildren() follow symlinks */
#define SPL_FILE_DIR_KEY_MODE_MASK                                             \
  0x00000F00 /* mask RecursiveDirectoryTree::key() */
#define SPL_FILE_DIR_KEY(intern, mode)                                         \
  ((intern->flags & SPL_FILE_DIR_KEY_MODE_MASK) == mode)

#define SPL_FILE_DIR_SKIPDOTS                                                  \
  0x00001000 /* Tells whether it should skip dots or not */
#define SPL_FILE_DIR_UNIXPATHS                                                 \
  0x00002000 /* Whether to unixify path separators */
