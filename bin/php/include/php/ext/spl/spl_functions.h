...
#define REGISTER_SPL_SUB_CLASS_EX(class_name, parent_class_name, obj_ctor,     \
                                  funcs)                                       \
  spl_register_sub_class(&spl_ce_##class_name, spl_ce_##parent_class_name,     \
                         #class_name, obj_ctor, funcs);

#define REGISTER_SPL_INTERFACE(class_name)                                     \
  spl_register_interface(&spl_ce_##class_name, #class_name,                    \
                         spl_funcs_##class_name);
...
#define REGISTER_SPL_PROPERTY(class_name, prop_name, prop_flags)               \
  spl_register_property(spl_ce_##class_name, prop_name, sizeof(prop_name) - 1, \
                        prop_flags);
...
void spl_register_sub_class(zend_class_entry **ppce,
                            zend_class_entry *parent_ce, char *class_name,
                            create_object_func_t ctor,
                            const zend_function_entry *function_list);
...
void spl_register_property(zend_class_entry *class_entry, char *prop_name,
                           int prop_name_len, int prop_flags);
 ...
 */
void spl_add_class_name(zval *list, zend_class_entry *pce, int allow,
                        int ce_flags);
void spl_add_interfaces(zval *list, zend_class_entry *pce, int allow,
                        int ce_flags);
void spl_add_traits(zval *list, zend_class_entry *pce, int allow, int ce_flags);
int spl_add_classes(zend_class_entry *pce, zval *list, int sub, int allow,
                    int ce_flags);

/* caller must efree(return) */
zend_string *spl_gen_private_prop_name(zend_class_entry *ce, char *prop_name,
                                       int prop_len);
...
#define SPL_MA(class_name, function_name, alias_class, alias_function,         \
               arg_info, flags)                                                \
