...
#define MYSQLND_MAX_ALLOWED_USER_LEN                                           \
  252 /* 63 char * 4byte . MySQL supports now only 16 char, but let it be      \
         forward compatible */
#define MYSQLND_MAX_ALLOWED_DB_LEN                                             \
  1024 /* 256 char * 4byte. MySQL supports now only 64 char in the tables, but \
          on the FS could be different. Forward compatible. */
...
#define CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA                                  \
  (1UL << 21) /* Enable authentication response packet to be larger than 255   \
                 bytes. */
#define CLIENT_CAN_HANDLE_EXPIRED_PASSWORDS                                    \
  (1UL << 22) /* Don't close the connection for a connection with expired      \
                 password. */
#define CLIENT_SESSION_TRACK (1UL << 23) /* Extended OK */
/*
  This is a mysqlnd extension. CLIENT_ODBC is not used anyway. We will reuse it
  for our case and translate it to not using SSL peer verification
*/
...
typedef enum func_status { PASS = 0, FAIL = 1 } enum_func_status;
...
#define IS_NUM(t)                                                              \
  ((t) <= FIELD_TYPE_INT24 || (t) == FIELD_TYPE_YEAR ||                        \
