...
struct st_mysqlnd_debug_methods {
  ...
  enum_func_status (*log)(MYSQLND_DEBUG *self, unsigned int line,
                          const char *const file, unsigned int level,
                          const char *type, const char *message);
  enum_func_status (*log_va)(MYSQLND_DEBUG *self, unsigned int line,
                             const char *const file, unsigned int level,
                             const char *type, const char *format, ...);
  zend_bool (*func_enter)(MYSQLND_DEBUG *self, unsigned int line,
                          const char *const file, const char *const func_name,
                          unsigned int func_name_len);
  enum_func_status (*func_leave)(MYSQLND_DEBUG *self, unsigned int line,
                                 const char *const file, uint64_t call_time);
...
#define DBG_PROFILE_START_TIME()                                               \
  gettimeofday(&__dbg_prof_tp, NULL);                                          \
  __dbg_prof_start = DBG_PROFILE_TIMEVAL_TO_DOUBLE(__dbg_prof_tp);
#define DBG_PROFILE_END_TIME(duration)                                         \
  gettimeofday(&__dbg_prof_tp, NULL);                                          \
  (duration) =                                                                 \
      (DBG_PROFILE_TIMEVAL_TO_DOUBLE(__dbg_prof_tp) - __dbg_prof_start);
#else
...
#define DBG_INF_EX(dbg_obj, msg)                                               \
  do {                                                                         \
    if (dbg_skip_trace == FALSE && (dbg_obj))                                  \
      (dbg_obj)->m->log((dbg_obj), __LINE__, __FILE__, -1, "info : ", (msg));  \
  } while (0)
#define DBG_ERR_EX(dbg_obj, msg)                                               \
  do {                                                                         \
    if (dbg_skip_trace == FALSE && (dbg_obj))                                  \
      (dbg_obj)->m->log((dbg_obj), __LINE__, __FILE__, -1, "error: ", (msg));  \
  } while (0)
#define DBG_INF_FMT_EX(dbg_obj, ...)                                           \
  do {                                                                         \
    if (dbg_skip_trace == FALSE && (dbg_obj))                                  \
      (dbg_obj)->m->log_va((dbg_obj), __LINE__, __FILE__, -1,                  \
                           "info : ", __VA_ARGS__);                            \
  } while (0)
#define DBG_ERR_FMT_EX(dbg_obj, ...)                                           \
  do {                                                                         \
    if (dbg_skip_trace == FALSE && (dbg_obj))                                  \
      (dbg_obj)->m->log_va((dbg_obj), __LINE__, __FILE__, -1,                  \
                           "error: ", __VA_ARGS__);                            \
...
#define DBG_BLOCK_ENTER_EX(dbg_obj, block_name)                                \
  DBG_BLOCK_ENTER_EX2((dbg_obj), (MYSQLND_DEBUG *)NULL, (block_name))
#define DBG_BLOCK_LEAVE_EX(dbg_obj)                                            \
  DBG_BLOCK_LEAVE_EX2((dbg_obj), (MYSQLND_DEBUG *)NULL)
...
#define DBG_ENTER_EX(dbg_obj, func_name)                                       \
  DBG_ENTER_EX2((dbg_obj), (MYSQLND_DEBUG *)NULL, (func_name))
#define DBG_LEAVE_EX(dbg_obj, leave)                                           \
  DBG_LEAVE_EX2((dbg_obj), (MYSQLND_DEBUG *)NULL, leave)
...
#define DBG_ENTER_EX2(dbg_obj1, dbg_obj2, func_name)                           \
  ...
  if ((dbg_obj1)) {                                                            \
    dbg_skip_trace = !(dbg_obj1)->m->func_enter(                               \
        (dbg_obj1), __LINE__, __FILE__, func_name, strlen(func_name));         \
  }                                                                            \
  if ((dbg_obj2)) {                                                            \
    dbg_skip_trace |= !(dbg_obj2)->m->func_enter(                              \
        (dbg_obj2), __LINE__, __FILE__, func_name, strlen(func_name));         \
  }                                                                            \
  if (dbg_skip_trace) {                                                        \
...
#define DBG_LEAVE_EX2(dbg_obj1, dbg_obj2, leave)                               \
  do {                                                                         \
    ...
    if ((dbg_obj1)) {                                                          \
      (dbg_obj1)->m->func_leave((dbg_obj1), __LINE__, __FILE__,                \
                                this_call_duration);                           \
    }                                                                          \
    if ((dbg_obj2)) {                                                          \
      (dbg_obj2)->m->func_leave((dbg_obj2), __LINE__, __FILE__,                \
                                this_call_duration);                           \
    }                                                                          \
    leave                                                                      \
  } while (0);
...
static inline void DBG_ENTER_EX(MYSQLND_DEBUG *dbg_obj,
...
#define TRACE_ALLOC_ENTER(func_name)                                           \
  DBG_ENTER_EX2(MYSQLND_G(dbg), MYSQLND_G(trace_alloc), (func_name))
#define TRACE_ALLOC_BLOCK_ENTER(bname)                                         \
  DBG_BLOCK_ENTER_EX2(MYSQLND_G(dbg), MYSQLND_G(trace_alloc), (bname))
#define TRACE_ALLOC_RETURN(value)                                              \
  DBG_RETURN_EX2(MYSQLND_G(dbg), MYSQLND_G(trace_alloc), (value))
