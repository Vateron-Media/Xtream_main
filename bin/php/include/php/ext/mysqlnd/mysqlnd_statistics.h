...
#define MYSQLND_INC_STATISTIC_W_VALUE2(enabler, stats, statistic1, value1,     \
                                       statistic2, value2)                     \
  {                                                                            \
    MYSQLND_STATS *_p_s = (MYSQLND_STATS *)(stats);                            \
    if ((enabler) && _p_s) {                                                   \
...
#define MYSQLND_INC_STATISTIC_W_VALUE3(enabler, stats, statistic1, value1,     \
  ...
  {                                                                            \
    ...
    if ((enabler) && _p_s) {                                                   \
      ...
      if (_s2 != _p_s->count)                                                  \
        MYSQLND_UPDATE_VALUE_AND_CALL_TRIGGER(_p_s, _s2, v2);                  \
      if (_s3 != _p_s->count)                                                  \
        MYSQLND_UPDATE_VALUE_AND_CALL_TRIGGER(_p_s, _s3, v3);                  \
    }                                                                          \
  }
...
PHPAPI mysqlnd_stat_trigger mysqlnd_stats_set_trigger(
    MYSQLND_STATS *const stats, enum_mysqlnd_collected_stats stat,
    mysqlnd_stat_trigger trigger);
