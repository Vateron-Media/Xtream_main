...
PHPAPI unsigned int
mysqlnd_plugin_register_ex(struct st_mysqlnd_plugin_header *plugin);
...
PHPAPI const MYSQLND_CHARSET *
mysqlnd_find_charset_nr(const unsigned int charsetno);
PHPAPI const MYSQLND_CHARSET *
mysqlnd_find_charset_name(const char *const charsetname);

/* Connect */
#define mysqlnd_init(flags, persistent)                                        \
  mysqlnd_connection_init((flags), (persistent), NULL /*use default factory*/)
#define mysqlnd_connect(conn, host, user, pass, pass_len, db, db_len, port,    \
                        socket, mysql_flags, client_api_flags)                 \
  mysqlnd_connection_connect((conn), (host), (user), (pass), (pass_len), (db), \
                             (db_len), (port), (socket), (mysql_flags),        \
                             (client_api_flags))

PHPAPI MYSQLND *mysqlnd_connection_init(
    const size_t client_flags, const zend_bool persistent,
    MYSQLND_CLASS_METHODS_TYPE(mysqlnd_object_factory) * object_factory);
PHPAPI MYSQLND *mysqlnd_connection_connect(
    MYSQLND *conn, const char *const host, const char *const user,
    const char *const passwd, unsigned int passwd_len, const char *const db,
    unsigned int db_len, unsigned int port, const char *const socket_or_pipe,
    unsigned int mysql_flags, unsigned int client_api_flags);

#define mysqlnd_change_user(conn, user, passwd, db, silent)                    \
  ((conn)->data)                                                               \
      ->m->change_user((conn)->data, (user), (passwd), (db), (silent),         \
                       strlen((passwd)))
...
#define mysqlnd_fetch_into(result, flags, ret_val, ext)                        \
  (result)->m.fetch_into((result), (flags), (ret_val), (ext)ZEND_FILE_LINE_CC)
...
#define mysqlnd_result_fetch_field_data(res, offset, ret)                      \
  (res)->m.fetch_field_data((res), (offset), (ret))
...
#define mysqlnd_get_client_stats(values)                                       \
  _mysqlnd_get_client_stats(mysqlnd_global_stats, (values)ZEND_FILE_LINE_CC)
...
#define mysqlnd_async_query(conn, query_str, query_len)                        \
  ((conn)->data)                                                               \
      ->m->send_query((conn)->data, (query_str), (query_len),                  \
                      MYSQLND_SEND_QUERY_EXPLICIT, NULL, NULL)
#define mysqlnd_reap_async_query(conn)                                         \
  ((conn)->data)->m->reap_query((conn)->data, MYSQLND_REAP_RESULT_EXPLICIT)
#define mysqlnd_unbuffered_skip_result(result) (result)->m.skip_result((result))

PHPAPI enum_func_status mysqlnd_poll(MYSQLND **r_array, MYSQLND **e_array,
                                     MYSQLND ***dont_poll, long sec, long usec,
                                     int *desc_num);

#define mysqlnd_use_result(conn) ((conn)->data)->m->use_result((conn)->data, 0)
#define mysqlnd_store_result(conn)                                             \
  ((conn)->data)->m->store_result((conn)->data, MYSQLND_STORE_NO_COPY)
#define mysqlnd_store_result_ofs(conn)                                         \
  ((conn)->data)->m->store_result((conn)->data, MYSQLND_STORE_COPY)
...
#define mysqlnd_ssl_set(conn, key, cert, ca, capath, cipher)                   \
  ((conn)->data)                                                               \
      ->m->ssl_set((conn)->data, (key), (cert), (ca), (capath), (cipher))
...
PHPAPI const char *
mysqlnd_field_type_name(const enum mysqlnd_field_types field_type);
...
#define mysqlnd_list_dbs(conn, wild)                                           \
  ((conn)->data)                                                               \
      ->m->list_method((conn)->data,                                           \
                       wild ? "SHOW DATABASES LIKE %s" : "SHOW DATABASES",     \
                       (wild), NULL)
...
#define mysqlnd_list_tables(conn, wild)                                        \
  ((conn)->data)                                                               \
      ->m->list_method((conn)->data,                                           \
                       wild ? "SHOW TABLES LIKE %s" : "SHOW TABLES", (wild),   \
                       NULL)
...
#define mysqlnd_shutdown(conn, level)                                          \
  ((conn)->data)->m->shutdown_server((conn)->data, (level))
...
#define MYSQLND_INC_GLOBAL_STATISTIC(statistic)                                \
  MYSQLND_INC_STATISTIC(MYSQLND_G(collect_statistics), mysqlnd_global_stats,   \
                        (statistic))

#define MYSQLND_DEC_GLOBAL_STATISTIC(statistic)                                \
  MYSQLND_DEC_STATISTIC(MYSQLND_G(collect_statistics), mysqlnd_global_stats,   \
                        (statistic))

#define MYSQLND_INC_GLOBAL_STATISTIC_W_VALUE2(statistic1, value1, statistic2,  \
                                              value2)                          \
  MYSQLND_INC_STATISTIC_W_VALUE2(MYSQLND_G(collect_statistics),                \
                                 mysqlnd_global_stats, (statistic1), (value1), \
                                 (statistic2), (value2))

#define MYSQLND_INC_CONN_STATISTIC(conn_stats, statistic)                      \
  MYSQLND_INC_STATISTIC(MYSQLND_G(collect_statistics), mysqlnd_global_stats,   \
                        (statistic));                                          \
  MYSQLND_INC_STATISTIC(MYSQLND_G(collect_statistics), (conn_stats),           \
                        (statistic));

#define MYSQLND_INC_CONN_STATISTIC_W_VALUE(conn_stats, statistic, value)       \
  MYSQLND_INC_STATISTIC_W_VALUE(MYSQLND_G(collect_statistics),                 \
                                mysqlnd_global_stats, (statistic), (value));   \
  MYSQLND_INC_STATISTIC_W_VALUE(MYSQLND_G(collect_statistics), (conn_stats),   \
                                (statistic), (value));
...
#define MYSQLND_INC_CONN_STATISTIC_W_VALUE2(conn_stats, statistic1, value1,    \
  ...
  MYSQLND_INC_STATISTIC_W_VALUE2(MYSQLND_G(collect_statistics),                \
                                 mysqlnd_global_stats, (statistic1), (value1), \
                                 (statistic2), (value2));                      \
  MYSQLND_INC_STATISTIC_W_VALUE2(MYSQLND_G(collect_statistics), (conn_stats),  \
                                 (statistic1), (value1), (statistic2),         \
                                 (value2));

#define MYSQLND_INC_CONN_STATISTIC_W_VALUE3(                                   \
    conn_stats, statistic1, value1, statistic2, value2, statistic3, value3)    \
  MYSQLND_INC_STATISTIC_W_VALUE3(                                              \
      MYSQLND_G(collect_statistics), mysqlnd_global_stats, (statistic1),       \
      (value1), (statistic2), (value2), (statistic3), (value3));               \
  MYSQLND_INC_STATISTIC_W_VALUE3(MYSQLND_G(collect_statistics), (conn_stats),  \
                                 (statistic1), (value1), (statistic2),         \
