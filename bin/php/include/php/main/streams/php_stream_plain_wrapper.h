...
PHPAPI php_stream *_php_stream_fopen(const char *filename, const char *mode,
...
#define php_stream_fopen_from_file(file, mode)                                 \
  _php_stream_fopen_from_file((file), (mode)STREAMS_CC)

PHPAPI php_stream *
_php_stream_fopen_from_fd(int fd, const char *mode,
                          const char *persistent_id STREAMS_DC);
#define php_stream_fopen_from_fd(fd, mode, persistent_id)                      \
  _php_stream_fopen_from_fd((fd), (mode), (persistent_id)STREAMS_CC)

PHPAPI php_stream *_php_stream_fopen_from_pipe(FILE *file,
                                               const char *mode STREAMS_DC);
#define php_stream_fopen_from_pipe(file, mode)                                 \
  _php_stream_fopen_from_pipe((file), (mode)STREAMS_CC)
...
#define php_stream_fopen_temporary_file(dir, pfx, opened_path)                 \
  _php_stream_fopen_temporary_file((dir), (pfx), (opened_path)STREAMS_CC)
...
PHPAPI FILE *
_php_stream_open_wrapper_as_file(char *path, char *mode, int options,
                                 zend_string **opened_path STREAMS_DC);
#define php_stream_open_wrapper_as_file(path, mode, options, opened_path)      \
  _php_stream_open_wrapper_as_file((path), (mode), (options),                  \
                                   (opened_path)STREAMS_CC)
