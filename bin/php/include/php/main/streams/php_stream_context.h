...
#define php_stream_context_from_zval(zcontext, nocontext)                      \
  ((zcontext)            ? zend_fetch_resource_ex(zcontext, "Stream-Context",  \
  ...
  do {                                                                         \
    if ((context) && (context)->notifier) {                                    \
      php_stream_notification_notify((context), (code),                        \
                                     PHP_STREAM_NOTIFY_SEVERITY_INFO, (xmsg),  \
                                     (xcode), 0, 0, NULL);                     \
    }                                                                          \
  } while (0)
...
#define php_stream_notify_progress(context, bsofar, bmax)                      \
  do {                                                                         \
    if ((context) && (context)->notifier) {                                    \
      php_stream_notification_notify((context), PHP_STREAM_NOTIFY_PROGRESS,    \
                                     PHP_STREAM_NOTIFY_SEVERITY_INFO, NULL, 0, \
                                     (bsofar), (bmax), NULL);                  \
    }                                                                          \
  } while (0)
...
#define php_stream_notify_progress_init(context, sofar, bmax)                  \
  do {                                                                         \
    if ((context) && (context)->notifier) {                                    \
      ...
      php_stream_notify_progress((context), (sofar), (bmax));                  \
    }                                                                          \
  } while (0)
...
#define php_stream_notify_progress_increment(context, dsofar, dmax)            \
  do {                                                                         \
    if ((context) && (context)->notifier &&                                    \
      ...
      php_stream_notify_progress((context), (context)->notifier->progress,     \
                                 (context)->notifier->progress_max);           \
    }                                                                          \
  } while (0)
...
#define php_stream_notify_file_size(context, file_size, xmsg, xcode)           \
  do {                                                                         \
    if ((context) && (context)->notifier) {                                    \
      php_stream_notification_notify((context),                                \
                                     PHP_STREAM_NOTIFY_FILE_SIZE_IS,           \
                                     PHP_STREAM_NOTIFY_SEVERITY_INFO, (xmsg),  \
                                     (xcode), 0, (file_size), NULL);           \
    }                                                                          \
  } while (0)
...
#define php_stream_notify_error(context, code, xmsg, xcode)                    \
  do {                                                                         \
    if ((context) && (context)->notifier) {                                    \
      php_stream_notification_notify((context), (code),                        \
                                     PHP_STREAM_NOTIFY_SEVERITY_ERR, (xmsg),   \
                                     (xcode), 0, 0, NULL);                     \
    }                                                                          \
  } while (0)
