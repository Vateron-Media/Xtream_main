...
#define php_stream_context_from_zval(zcontext, nocontext) (                                                                            \
	(zcontext) ? zend_fetch_resource_ex(zcontext, "Stream-Context", php_le_stream_context()) : (nocontext)		 ? NULL                \
	...
	{                                                                                                                        \
		...
		{                                                                                                                    \
			php_stream_notification_notify((context), PHP_STREAM_NOTIFY_COMPLETED, PHP_STREAM_NOTIFY_SEVERITY_INFO,          \
										   NULL, 0, (context)->notifier->progress, (context)->notifier->progress_max, NULL); \
		}                                                                                                                    \
	} while (0)
...
#define php_stream_notify_progress_increment(context, dsofar, dmax)                                                  \
	do                                                                                                               \
	{                                                                                                                \
		if ((context) && (context)->notifier && ((context)->notifier->mask & PHP_STREAM_NOTIFIER_PROGRESS))          \
		{                                                                                                            \
			(context)->notifier->progress += (dsofar);                                                               \
			(context)->notifier->progress_max += (dmax);                                                             \
			php_stream_notify_progress((context), (context)->notifier->progress, (context)->notifier->progress_max); \
		}                                                                                                            \
	} while (0)
