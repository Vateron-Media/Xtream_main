...
typedef enum {
  ...
  PSFS_FEED_ME,   /* filter needs more data; stop processing chain until more is
                     available */
...
/* Buckets API. */
BEGIN_EXTERN_C()
PHPAPI php_stream_bucket *php_stream_bucket_new(php_stream *stream, char *buf,
                                                size_t buflen, uint8_t own_buf,
...
#define php_stream_filter_alloc(fops, thisptr, persistent)                     \
  _php_stream_filter_alloc((fops), (thisptr), (persistent)STREAMS_CC)
...
PHPAPI int php_stream_filter_register_factory_volatile(
