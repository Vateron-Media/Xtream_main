...
#define php_stream_memory_create(mode)                                         \
  _php_stream_memory_create((mode)STREAMS_CC)
#define php_stream_memory_create_rel(mode)                                     \
  _php_stream_memory_create((mode)STREAMS_REL_CC)
#define php_stream_memory_open(mode, buf, length)                              \
  _php_stream_memory_open((mode), (buf), (length)STREAMS_CC)
#define php_stream_memory_get_buffer(stream, length)                           \
  _php_stream_memory_get_buffer((stream), (length)STREAMS_CC)

#define php_stream_temp_new()                                                  \
  php_stream_temp_create(TEMP_STREAM_DEFAULT, PHP_STREAM_MAX_MEM)
...
#define php_stream_temp_create_rel(mode, max_memory_usage)                     \
  _php_stream_temp_create((mode), (max_memory_usage)STREAMS_REL_CC)
...
PHPAPI php_stream *_php_stream_memory_open(int mode, char *buf,
...
PHPAPI php_stream *_php_stream_temp_create_ex(int mode, size_t max_memory_usage,
                                              const char *tmpdir STREAMS_DC);
PHPAPI php_stream *_php_stream_temp_open(int mode, size_t max_memory_usage,
                                         char *buf, size_t length STREAMS_DC);
