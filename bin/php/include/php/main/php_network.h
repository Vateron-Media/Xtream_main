 ...
 * timeouttv follows same rules as select(2), but is reduced to millisecond
 * accuracy. Returns 0 on timeout, -1 on error, or the event mask (ala poll(2)).
 */
static inline int php_pollfd_for(php_socket_t fd, int events,
...
#define PHP_SAFE_MAX_FD(m, n)                                                  \
  do {                                                                         \
    if (n + 1 >= FD_SETSIZE) {                                                 \
      _php_emit_fd_setsize_warning(n);                                         \
    }                                                                          \
  } while (0)
#else
...
#define PHP_SAFE_MAX_FD(m, n)                                                  \
  do {                                                                         \
    if (m >= FD_SETSIZE) {                                                     \
      _php_emit_fd_setsize_warning(m);                                         \
      m = FD_SETSIZE - 1;                                                      \
    }                                                                          \
  } while (0)
#endif
...
BEGIN_EXTERN_C()
PHPAPI int php_network_getaddresses(const char *host, int socktype,
                                    struct sockaddr ***sal,
                                    zend_string **error_string);
PHPAPI void php_network_freeaddresses(struct sockaddr **sal);

PHPAPI php_socket_t php_network_connect_socket_to_host(
    const char *host, unsigned short port, int socktype, int asynchronous,
    struct timeval *timeout, zend_string **error_string, int *error_code,
    char *bindto, unsigned short bindport, long sockopts);

PHPAPI int php_network_connect_socket(php_socket_t sockfd,
                                      const struct sockaddr *addr,
                                      socklen_t addrlen, int asynchronous,
                                      struct timeval *timeout,
                                      zend_string **error_string,
                                      int *error_code);

#define php_connect_nonb(sock, addr, addrlen, timeout)                         \
  php_network_connect_socket((sock), (addr), (addrlen), 0, (timeout), NULL,    \
                             NULL)

PHPAPI php_socket_t php_network_bind_socket_to_local_addr(
    const char *host, unsigned port, int socktype, long sockopts,
    zend_string **error_string, int *error_code);

PHPAPI php_socket_t php_network_accept_incoming(
    php_socket_t srvsock, zend_string **textaddr, struct sockaddr **addr,
    socklen_t *addrlen, struct timeval *timeout, zend_string **error_string,
    int *error_code, int tcp_nodelay);

PHPAPI int php_network_get_sock_name(php_socket_t sock, zend_string **textaddr,
                                     struct sockaddr **addr,
                                     socklen_t *addrlen);

PHPAPI int php_network_get_peer_name(php_socket_t sock, zend_string **textaddr,
                                     struct sockaddr **addr,
                                     socklen_t *addrlen);

PHPAPI void php_any_addr(int family, php_sockaddr_storage *addr,
                         unsigned short port);
...
BEGIN_EXTERN_C()
PHPAPI php_stream *
_php_stream_sock_open_from_socket(php_socket_t socket,
                                  const char *persistent_id STREAMS_DC);
/* open a connection to a host using php_hostconnect and return a stream */
PHPAPI php_stream *
_php_stream_sock_open_host(const char *host, unsigned short port, int socktype,
                           struct timeval *timeout,
                           const char *persistent_id STREAMS_DC);
...
#define php_stream_sock_open_from_socket(socket, persistent)                   \
  _php_stream_sock_open_from_socket((socket), (persistent)STREAMS_CC)
#define php_stream_sock_open_host(host, port, socktype, timeout, persistent)   \
  _php_stream_sock_open_host((host), (port), (socktype), (timeout),            \
                             (persistent)STREAMS_CC)

/* {{{ memory debug */
#define php_stream_sock_open_from_socket_rel(socket, persistent)               \
  _php_stream_sock_open_from_socket((socket), (persistent)STREAMS_REL_CC)
#define php_stream_sock_open_host_rel(host, port, socktype, timeout,           \
                                      persistent)                              \
  _php_stream_sock_open_host((host), (port), (socktype), (timeout),            \
                             (persistent)STREAMS_REL_CC)
#define php_stream_sock_open_unix_rel(path, pathlen, persistent, timeval)      \
  _php_stream_sock_open_unix((path), (pathlen), (persistent),                  \
                             (timeval)STREAMS_REL_CC)
