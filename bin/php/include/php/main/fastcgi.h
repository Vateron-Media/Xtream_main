...
#define FCGI_HASH_FUNC(var, var_len)                                           \
  (UNEXPECTED(var_len < 3)                                                     \
       ? (unsigned int)var_len                                                 \
       : (((unsigned int)var[3]) << 2) +                                       \
             (((unsigned int)var[var_len - 2]) << 4) +                         \
             (((unsigned int)var[var_len - 1]) << 2) + var_len)

#define FCGI_GETENV(request, name)                                             \
  fcgi_quick_getenv(request, name, sizeof(name) - 1,                           \
                    FCGI_HASH_FUNC(name, sizeof(name) - 1))

#define FCGI_PUTENV(request, name, value)                                      \
  fcgi_quick_putenv(request, name, sizeof(name) - 1,                           \
                    FCGI_HASH_FUNC(name, sizeof(name) - 1), value)
...
fcgi_request *fcgi_init_request(int listen_socket, void (*on_accept)(),
                                void (*on_read)(), void (*on_close)());
