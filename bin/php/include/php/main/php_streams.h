...
#define STREAMS_D                                                              \
  int __php_stream_call_depth ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC
#define STREAMS_C 0 ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC
#define STREAMS_REL_C                                                          \
  __php_stream_call_depth + 1 ZEND_FILE_LINE_CC,                               \
...
/* these functions relay the file/line number information. They are depth aware,
 * so they will pass the ultimate ancestor, which is useful, because there can
 * be several layers of calls */
...
#define php_stream_fopen_with_path_rel(filename, mode, path, opened, options)  \
  _php_stream_fopen_with_path((filename), (mode), (path), (opened),            \
                              (options)STREAMS_REL_CC)
...
#define php_stream_fopen_from_file_rel(file, mode)                             \
  _php_stream_fopen_from_file((file), (mode)STREAMS_REL_CC)

#define php_stream_fopen_from_pipe_rel(file, mode)                             \
  _php_stream_fopen_from_pipe((file), (mode)STREAMS_REL_CC)

#define php_stream_fopen_tmpfile_rel()                                         \
  _php_stream_fopen_tmpfile(0 STREAMS_REL_CC)
...
#define php_stream_open_wrapper_rel(path, mode, options, opened)               \
  _php_stream_open_wrapper_ex((path), (mode), (options), (opened),             \
                              NULL STREAMS_REL_CC)
...
struct _php_stream {
  ...
  struct _php_stream *
      enclosing_stream; /* this is a private stream owned by enclosing_stream */
}; /* php_stream */
...
#define php_stream_to_zval(stream, zval)                                       \
  {                                                                            \
    ZVAL_RES(zval, (stream)->res);                                             \
    (stream)->__exposed = 1;                                                   \
  }

#define php_stream_from_zval(xstr, pzval)                                      \
  do {                                                                         \
    if (((xstr) = (php_stream *)zend_fetch_resource2_ex(                       \
             (pzval), "stream", php_file_le_stream(),                          \
             php_file_le_pstream())) == NULL) {                                \
      RETURN_FALSE;                                                            \
    }                                                                          \
  } while (0)
#define php_stream_from_res(xstr, res)                                         \
  do {                                                                         \
    if (((xstr) = (php_stream *)zend_fetch_resource2(                          \
             (res), "stream", php_file_le_stream(), php_file_le_pstream())) == \
        NULL) {                                                                \
      RETURN_FALSE;                                                            \
    }                                                                          \
  } while (0)
#define php_stream_from_res_no_verify(xstr, pzval)                             \
  (xstr) = (php_stream *)zend_fetch_resource2(                                 \
      (res), "stream", php_file_le_stream(), php_file_le_pstream())
#define php_stream_from_zval_no_verify(xstr, pzval)                            \
  (xstr) = (php_stream *)zend_fetch_resource2_ex(                              \
      (pzval), "stream", php_file_le_stream(), php_file_le_pstream())
...
#define PHP_STREAM_FREE_PERSISTENT                                             \
  16 /* manually freeing a persistent connection */
#define PHP_STREAM_FREE_IGNORE_ENCLOSING                                       \
  32 /* don't close the enclosing stream instead */
#define PHP_STREAM_FREE_KEEP_RSRC 64 /* keep associated zend_resource */
#define PHP_STREAM_FREE_CLOSE                                                  \
  (PHP_STREAM_FREE_CALL_DTOR | PHP_STREAM_FREE_RELEASE_STREAM)
#define PHP_STREAM_FREE_CLOSE_CASTED                                           \
  (PHP_STREAM_FREE_CLOSE | PHP_STREAM_FREE_PRESERVE_HANDLE)
#define PHP_STREAM_FREE_CLOSE_PERSISTENT                                       \
  (PHP_STREAM_FREE_CLOSE | PHP_STREAM_FREE_PERSISTENT)
...
#define php_stream_close(stream)                                               \
  _php_stream_free((stream), PHP_STREAM_FREE_CLOSE)
#define php_stream_pclose(stream)                                              \
  _php_stream_free((stream), PHP_STREAM_FREE_CLOSE_PERSISTENT)
...
#define php_stream_gets(stream, buf, maxlen)                                   \
  _php_stream_get_line((stream), (buf), (maxlen), NULL)

#define php_stream_get_line(stream, buf, maxlen, retlen)                       \
  _php_stream_get_line((stream), (buf), (maxlen), (retlen))
PHPAPI zend_string *php_stream_get_record(php_stream *stream, size_t maxlen,
...
PHPAPI int _php_stream_stat_path(const char *path, int flags,
                                 php_stream_statbuf *ssb,
                                 php_stream_context *context);
...
PHPAPI int _php_stream_mkdir(const char *path, int mode, int options,
                             php_stream_context *context);
#define php_stream_mkdir(path, mode, options, context)                         \
  _php_stream_mkdir(path, mode, options, context)

PHPAPI int _php_stream_rmdir(const char *path, int options,
                             php_stream_context *context);
#define php_stream_rmdir(path, options, context)                               \
  _php_stream_rmdir(path, options, context)

PHPAPI php_stream *_php_stream_opendir(const char *path, int options,
                                       php_stream_context *context STREAMS_DC);
#define php_stream_opendir(path, options, context)                             \
  _php_stream_opendir((path), (options), (context)STREAMS_CC)
...
PHPAPI int _php_stream_scandir(const char *dirname, zend_string **namelist[],
                               int flags, php_stream_context *context,
                               int (*compare)(const zend_string **a,
                                              const zend_string **b));
...
#define php_stream_set_chunk_size(stream, size)                                \
  _php_stream_set_option((stream), PHP_STREAM_OPTION_SET_CHUNK_SIZE, (size),   \
                         NULL)
...
/* change the buffering mode of stream. value is a PHP_STREAM_BUFFER_XXXX value,
 * ptrparam is a ptr to a size_t holding the required buffer size */
...
#define php_stream_supports_lock(stream)                                       \
  (_php_stream_set_option((stream), PHP_STREAM_OPTION_LOCKING, 0,              \
                          (void *)PHP_STREAM_LOCK_SUPPORTED) == 0              \
       ? 1                                                                     \
       : 0)
#define php_stream_lock(stream, mode)                                          \
  _php_stream_set_option((stream), PHP_STREAM_OPTION_LOCKING, (mode),          \
                         (void *)NULL)
...
#define php_stream_truncate_supported(stream)                                  \
  (_php_stream_set_option((stream), PHP_STREAM_OPTION_TRUNCATE_API,            \
                          ...
                          NULL) == PHP_STREAM_OPTION_RETURN_OK                 \
       ? 1                                                                     \
       : 0)
...
#define php_stream_truncate_set_size(stream, size)                             \
  _php_stream_truncate_set_size((stream), (size))
END_EXTERN_C()

#define PHP_STREAM_OPTION_META_DATA_API                                        \
  11 /* ptrparam is a zval* to which to add meta data information */
#define php_stream_populate_meta_data(stream, zv)                              \
  (_php_stream_set_option((stream), PHP_STREAM_OPTION_META_DATA_API, 0, zv) == \
...
PHPAPI int _php_stream_copy_to_stream_ex(php_stream *src, php_stream *dest,
                                         size_t maxlen, size_t *len STREAMS_DC);
 ...
 * when done. */
PHPAPI zend_string *_php_stream_copy_to_mem(php_stream *src, size_t maxlen,
                                            int persistent STREAMS_DC);
#define php_stream_copy_to_mem(src, maxlen, persistent)                        \
  _php_stream_copy_to_mem((src), (maxlen), (persistent)STREAMS_CC)
...
/* try really, really hard to make sure the cast happens (avoid using this flag
 * if possible) */
#define PHP_STREAM_CAST_TRY_HARD 0x80000000
#define PHP_STREAM_CAST_RELEASE                                                \
  0x40000000 /* stream becomes invalid on success */
...
BEGIN_EXTERN_C()
PHPAPI int php_register_url_stream_wrapper(const char *protocol,
...
PHPAPI php_stream *
_php_stream_open_wrapper_ex(const char *path, const char *mode, int options,
                            zend_string **opened_path,
                            php_stream_context *context STREAMS_DC);
...
PHPAPI void php_stream_wrapper_log_error(const php_stream_wrapper *wrapper,
                                         int options, const char *fmt, ...)
    PHP_ATTRIBUTE_FORMAT(printf, 3, 4);

#define PHP_STREAM_UNCHANGED 0 /* orig stream was seekable anyway */
#define PHP_STREAM_RELEASED                                                    \
  1 /* newstream should be used; origstream is no longer valid */
#define PHP_STREAM_FAILED 2 /* an error occurred while attempting conversion   \
                             */
#define PHP_STREAM_CRITICAL                                                    \
  3 /* an error occurred; origstream is in an unknown state; you should close  \
       origstream */
