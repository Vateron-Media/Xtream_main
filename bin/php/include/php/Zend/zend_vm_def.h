...
ZEND_VM_COLD_CONSTCONST_HANDLER(6, ZEND_SL, CONST | TMPVARCV, CONST | TMPVARCV)
{
	...
	else if (EXPECTED(Z_TYPE_INFO_P(op1) == IS_LONG) && EXPECTED(Z_TYPE_INFO_P(op2) == IS_LONG) && EXPECTED((zend_ulong)Z_LVAL_P(op2) < SIZEOF_ZEND_LONG * 8))
	{
...
ZEND_VM_HOT_NOCONSTCONST_HANDLER(21, ZEND_IS_SMALLER_OR_EQUAL, CONST | TMPVARCV, CONST | TMPVARCV, SPEC(SMART_BRANCH))
{
	...
	else if (EXPECTED(Z_TYPE_INFO_P(op1) == IS_LONG))
	{
		if (EXPECTED(Z_TYPE_INFO_P(op2) == IS_LONG))
		{
			...
			{
				ZEND_VM_C_LABEL(is_smaller_or_equal_false) : ZEND_VM_SMART_BRANCH_FALSE();
...
ZEND_VM_HANDLER(28, ZEND_ASSIGN_OBJ_OP, VAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, OP)
{
	...
	do
	{
		...
		ZEND_VM_C_LABEL(assign_op_object) : /* here we are sure we are dealing with an object */
											zobj = Z_OBJ_P(object);
...
ZEND_VM_HANDLER(27, ZEND_ASSIGN_DIM_OP, VAR | CV, CONST | TMPVAR | UNUSED | NEXT | CV, OP)
{
	...
	if (EXPECTED(Z_TYPE_P(container) == IS_ARRAY))
	{
		...
		ZEND_VM_C_LABEL(assign_dim_op_new_array) : dim = GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R);
	...
	else
	{
		...
		else
		{
			...
			ZEND_VM_C_LABEL(assign_dim_op_ret_null) : FREE_OP_DATA();
			if (UNEXPECTED(RETURN_VALUE_USED(opline)))
			{
				ZVAL_NULL(EX_VAR(opline->result.var));
			}
...
ZEND_VM_HANDLER(132, ZEND_PRE_INC_OBJ, VAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, CACHE_SLOT)
{
	...
	do
	{
		...
		ZEND_VM_C_LABEL(pre_incdec_object) : /* here we are sure we are dealing with an object */
											 zobj = Z_OBJ_P(object);
		if (OP2_TYPE == IS_CONST)
		{
			name = Z_STR_P(property);
...
ZEND_VM_HANDLER(134, ZEND_POST_INC_OBJ, VAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, CACHE_SLOT)
{
	...
	do
	{
		...
		ZEND_VM_C_LABEL(post_incdec_object) : /* here we are sure we are dealing with an object */
											  zobj = Z_OBJ_P(object);
		if (OP2_TYPE == IS_CONST)
		{
			name = Z_STR_P(property);
...
ZEND_VM_HELPER(zend_fetch_var_address_helper, CONST | TMPVAR | CV, UNUSED, int type)
{
	...
	if (retval == NULL)
	{
		...
		{
			ZEND_VM_C_LABEL(fetch_this) : zend_fetch_this_var(type OPLINE_CC EXECUTE_DATA_CC);
			if (OP1_TYPE != IS_CONST)
			{
				zend_tmp_string_release(tmp_name);
			}
...
{
	int fetch_type =
		(UNEXPECTED(ZEND_CALL_INFO(EX(call)) & ZEND_CALL_SEND_ARG_BY_REF)) ? BP_VAR_W : BP_VAR_R;
...
ZEND_VM_COLD_CONSTCONST_HANDLER(81, ZEND_FETCH_DIM_R, CONST | TMPVAR | CV, CONST | TMPVAR | CV)
{
	...
	if (OP1_TYPE != IS_CONST)
	{
		if (EXPECTED(Z_TYPE_P(container) == IS_ARRAY))
		{
			ZEND_VM_C_LABEL(fetch_dim_r_array) : value = zend_fetch_dimension_address_inner(Z_ARRVAL_P(container), dim, OP2_TYPE, BP_VAR_R EXECUTE_DATA_CC);
		...
		{
			ZEND_VM_C_LABEL(fetch_dim_r_slow) : if (OP2_TYPE == IS_CONST && Z_EXTRA_P(dim) == ZEND_EXTRA_VALUE)
			{
				dim++;
			}
	...
	do
	{
		...
		if (OP2_TYPE == IS_CONST)
		{
			...
			if (EXPECTED(zobj->ce == CACHED_PTR_EX(cache_slot)))
			{
				...
				{
					ZEND_VM_C_LABEL(fetch_obj_r_simple) : retval = OBJ_PROP(zobj, prop_offset);
					if (EXPECTED(Z_TYPE_INFO_P(retval) != IS_UNDEF))
					{
						...
						{
							ZEND_VM_C_LABEL(fetch_obj_r_fast_copy) : ZVAL_COPY_DEREF(EX_VAR(opline->result.var), retval);
							ZEND_VM_NEXT_OPCODE();
						}
					}
				}
...
#if ZEND_DEBUG
		if (!EG(exception) && prop_info && prop_info != ZEND_WRONG_PROPERTY_INFO && ZEND_TYPE_IS_SET(prop_info->type))
		{
		...
		{
			ZEND_VM_C_LABEL(fetch_obj_r_copy) : ZVAL_COPY_DEREF(EX_VAR(opline->result.var), retval);
		}
	...
	do
	{
		...
		if (OP2_TYPE == IS_CONST)
		{
			...
			if (EXPECTED(zobj->ce == CACHED_PTR_EX(cache_slot)))
			{
				...
				if (EXPECTED(IS_VALID_PROPERTY_OFFSET(prop_offset)))
				{
					ZEND_VM_C_LABEL(fetch_obj_is_simple) : retval = OBJ_PROP(zobj, prop_offset);
					if (EXPECTED(Z_TYPE_P(retval) != IS_UNDEF))
					{
						...
						{
							ZEND_VM_C_LABEL(fetch_obj_is_fast_copy) : ZVAL_COPY_DEREF(EX_VAR(opline->result.var), retval);
							ZEND_VM_NEXT_OPCODE();
						}
					}
				}
		...
		if (retval != EX_VAR(opline->result.var))
		{
			ZEND_VM_C_LABEL(fetch_obj_is_copy) : ZVAL_COPY_DEREF(EX_VAR(opline->result.var), retval);
		}
		else if (UNEXPECTED(Z_ISREF_P(retval)))
		{
			zend_unwrap_reference(retval);
...
ZEND_VM_HANDLER(155, ZEND_FETCH_LIST_W, VAR, CONST | TMPVAR | CV)
{
	...
	if (OP1_TYPE == IS_VAR && Z_TYPE_P(EX_VAR(opline->op1.var)) != IS_INDIRECT && UNEXPECTED(!Z_ISREF_P(container)))
	{
...
ZEND_VM_HANDLER(24, ZEND_ASSIGN_OBJ, VAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, CACHE_SLOT, SPEC(OP_DATA = CONST | TMP | VAR | CV))
{
	...
	ZEND_VM_C_LABEL(assign_object) : zobj = Z_OBJ_P(object);
	if (OP2_TYPE == IS_CONST)
	{
		if (EXPECTED(zobj->ce == CACHED_PTR(opline->extended_value)))
		{
			...
			if (EXPECTED(IS_VALID_PROPERTY_OFFSET(prop_offset)))
			{
				...
				if (Z_TYPE_P(property_val) != IS_UNDEF)
				{
					...
					{
						ZEND_VM_C_LABEL(fast_assign_obj) : value = zend_assign_to_variable_ex(property_val, value, OP_DATA_TYPE, EX_USES_STRICT_TYPES(), &garbage);
						if (UNEXPECTED(RETURN_VALUE_USED(opline)))
						{
	...
	ZEND_VM_C_LABEL(free_and_exit_assign_obj) : if (UNEXPECTED(RETURN_VALUE_USED(opline)) && value)
	{
		ZVAL_COPY_DEREF(EX_VAR(opline->result.var), value);
	}
	FREE_OP_DATA();
	ZEND_VM_C_LABEL(exit_assign_obj) : if (garbage)
	{
...
ZEND_VM_HANDLER(23, ZEND_ASSIGN_DIM, VAR | CV, CONST | TMPVAR | UNUSED | NEXT | CV, SPEC(OP_DATA = CONST | TMP | VAR | CV))
{
	...
	{
		ZEND_VM_C_LABEL(try_assign_dim_array) : SEPARATE_ARRAY(object_ptr);
		if (OP2_TYPE == IS_UNUSED)
		{
	...
	else
	{
		...
		{
			if (Z_ISREF_P(orig_object_ptr) && ZEND_REF_HAS_TYPE_SOURCES(Z_REF_P(orig_object_ptr)) && !zend_verify_ref_array_assignable(Z_REF_P(orig_object_ptr)))
			{
				dim = GET_OP2_ZVAL_PTR(BP_VAR_R);
				FREE_OP_DATA();
				UNDEF_RESULT();
		...
		else
		{
			...
			dim = GET_OP2_ZVAL_PTR(BP_VAR_R);
			ZEND_VM_C_LABEL(assign_dim_error) : FREE_OP_DATA();
			if (UNEXPECTED(RETURN_VALUE_USED(opline)))
			{
				ZVAL_NULL(EX_VAR(opline->result.var));
			}
...
ZEND_VM_HANDLER(113, ZEND_INIT_STATIC_METHOD_CALL, UNUSED | CLASS_FETCH | CONST | VAR, CONST | TMPVAR | UNUSED | CONSTRUCTOR | CV, NUM | CACHE_SLOT)
{
	...
	else
	{
		...
		if (OP1_TYPE == IS_UNUSED && ((opline->op1.num & ZEND_FETCH_CLASS_MASK) == ZEND_FETCH_CLASS_PARENT ||
...
ZEND_VM_HANDLER(128, ZEND_INIT_DYNAMIC_CALL, ANY, CONST | TMPVAR | CV, NUM)
{
	...
	ZEND_VM_C_LABEL(try_function_name) : if (OP2_TYPE != IS_CONST && EXPECTED(Z_TYPE_P(function_name) == IS_STRING))
	{
...
ZEND_VM_HANDLER(139, ZEND_GENERATOR_CREATE, ANY, ANY)
{
	...
	if (EXPECTED(return_value))
	{
		...
		if ((call_info & Z_TYPE_MASK) == IS_OBJECT && (!(call_info & (ZEND_CALL_CLOSURE | ZEND_CALL_RELEASE_THIS))
													   /* Bug #72523 */
...
ZEND_VM_HOT_SEND_HANDLER(116, ZEND_SEND_VAL_EX, CONST | TMP, CONST | UNUSED | NUM, SPEC(QUICK_ARG))
{
	...
	{
		ZEND_VM_C_LABEL(send_val_by_ref) : ZEND_VM_DISPATCH_TO_HELPER(zend_cannot_pass_by_ref_helper, _arg_num, arg_num, _arg, arg);
	}
	value = GET_OP1_ZVAL_PTR(BP_VAR_R);
	ZVAL_COPY_VALUE(arg, value);
	if (OP1_TYPE == IS_CONST)
...
ZEND_VM_HOT_SEND_HANDLER(50, ZEND_SEND_VAR_NO_REF_EX, VAR, CONST | UNUSED | NUM, SPEC(QUICK_ARG))
{
	...
	ZEND_VM_C_LABEL(send_var) : varptr = GET_OP1_ZVAL_PTR(BP_VAR_R);
	if (UNEXPECTED(Z_ISREF_P(varptr)))
	{
		zend_refcounted *ref = Z_COUNTED_P(varptr);
...
ZEND_VM_HOT_SEND_HANDLER(66, ZEND_SEND_VAR_EX, VAR | CV, CONST | UNUSED | NUM, SPEC(QUICK_ARG))
{
	...
	{
		ZEND_VM_C_LABEL(send_var_by_ref) : varptr = GET_OP1_ZVAL_PTR_PTR(BP_VAR_W);
		if (Z_ISREF_P(varptr))
		{
			Z_ADDREF_P(varptr);
		}
...
ZEND_VM_HOT_SEND_HANDLER(100, ZEND_CHECK_FUNC_ARG, UNUSED, CONST | UNUSED | NUM, SPEC(QUICK_ARG))
{
	...
	if (OP2_TYPE == IS_CONST)
	{
		...
		arg_num = zend_get_arg_offset_by_name(
					  EX(call)->func, arg_name, CACHE_ADDR(opline->result.num)) +
				  1;
...
ZEND_VM_HANDLER(119, ZEND_SEND_ARRAY, ANY, ANY, NUM)
{
	...
	else
	{
		...
		if (OP2_TYPE != IS_UNUSED)
		{
			...
			else if (EX_USES_STRICT_TYPES() || !zend_parse_arg_long_weak(op2, &len, /* arg_num */ 3))
			{
				zend_type_error(
...
ZEND_VM_HOT_HANDLER(64, ZEND_RECV_INIT, NUM, CONST, CACHE_SLOT)
{
	...
	{
		ZEND_VM_C_LABEL(recv_init_check_type) : if ((EX(func)->op_array.fn_flags & ZEND_ACC_HAS_TYPE_HINTS) != 0)
		{
			SAVE_OPLINE();
...
ZEND_VM_COLD_CONST_HANDLER(110, ZEND_CLONE, CONST | TMPVAR | UNUSED | THIS | CV, ANY)
{
	...
	if (clone && !(clone->common.fn_flags & ZEND_ACC_PUBLIC))
	{
		...
		{
			if (UNEXPECTED(clone->common.fn_flags & ZEND_ACC_PRIVATE) || UNEXPECTED(!zend_check_protected(zend_get_function_root_class(clone), scope)))
			{
...
ZEND_VM_HANDLER(181, ZEND_FETCH_CLASS_CONSTANT, VAR | CONST | UNUSED | CLASS_FETCH, CONST | TMPVARCV, CACHE_SLOT)
{
	...
	do
	{
		...
		if (OP1_TYPE != IS_CONST && OP2_TYPE == IS_CONST && EXPECTED(CACHED_PTR(opline->extended_value) == ce))
		{
			value = CACHED_PTR(opline->extended_value + sizeof(void *));
			break;
		}
...
ZEND_VM_HANDLER(72, ZEND_ADD_ARRAY_ELEMENT, CONST | TMP | VAR | CV, CONST | TMPVAR | UNUSED | NEXT | CV, REF)
{
	...
	if (OP2_TYPE != IS_UNUSED)
	{
		...
		ZEND_VM_C_LABEL(add_again) : if (EXPECTED(Z_TYPE_P(offset) == IS_STRING))
		{
			...
			ZEND_VM_C_LABEL(str_index) : zend_hash_update(Z_ARRVAL_P(EX_VAR(opline->result.var)), str, expr_ptr);
		}
		else if (EXPECTED(Z_TYPE_P(offset) == IS_LONG))
		{
			hval = Z_LVAL_P(offset);
			ZEND_VM_C_LABEL(num_index) : zend_hash_index_update(Z_ARRVAL_P(EX_VAR(opline->result.var)), hval, expr_ptr);
		}
...
ZEND_VM_HANDLER(75, ZEND_UNSET_DIM, VAR | CV, CONST | TMPVAR | CV)
{
	...
	do
	{
		if (EXPECTED(Z_TYPE_P(container) == IS_ARRAY))
		{
			...
			ht = Z_ARRVAL_P(container);
			ZEND_VM_C_LABEL(offset_again) : if (EXPECTED(Z_TYPE_P(offset) == IS_STRING))
			{
				key = Z_STR_P(offset);
				if (OP2_TYPE != IS_CONST)
				{
				...
				ZEND_VM_C_LABEL(str_index_dim) : ZEND_ASSERT(ht != &EG(symbol_table));
...
ZEND_VM_COLD_CONST_HANDLER(125, ZEND_FE_RESET_RW, CONST | TMP | VAR | CV, JMP_ADDR)
{
	...
	else if (OP1_TYPE != IS_CONST && EXPECTED(Z_TYPE_P(array_ptr) == IS_OBJECT))
	{
		if (!Z_OBJCE_P(array_ptr)->get_iterator)
		{
			...
			if (Z_OBJ_P(array_ptr)->properties && UNEXPECTED(GC_REFCOUNT(Z_OBJ_P(array_ptr)->properties) > 1))
			{
		...
		while (1)
		{
			...
			if (EXPECTED(value_type != IS_UNDEF))
			{
				if (UNEXPECTED(value_type == IS_INDIRECT))
				{
					...
					value_type = Z_TYPE_INFO_P(value);
					if (EXPECTED(value_type != IS_UNDEF) && EXPECTED(zend_check_property_access(Z_OBJ_P(array), p->key, 0) == SUCCESS))
					{
						break;
					}
				}
				else if (EXPECTED(Z_OBJCE_P(array)->default_properties_count == 0) || !p->key || zend_check_property_access(Z_OBJ_P(array), p->key, 1) == SUCCESS)
				{
					break;
				}
			}
	...
	else
	{
		...
		if (EXPECTED(++iter->index > 0))
		{
			...
			if (UNEXPECTED(funcs->valid(iter) == FAILURE))
			{
				...
				ZEND_VM_C_LABEL(fe_fetch_r_exit) : ZEND_VM_SET_RELATIVE_OPCODE(opline, opline->extended_value);
				ZEND_VM_CONTINUE();
			}
		}
...
ZEND_VM_HANDLER(126, ZEND_FE_FETCH_RW, VAR, ANY, JMP_ADDR)
{
	...
	if (EXPECTED(Z_TYPE_P(array) == IS_ARRAY))
	{
		...
		else
		{
			...
			EG(ht_iterators)
			[Z_FE_ITER_P(EX_VAR(opline->op1.var))].pos = pos;
			if (RETURN_VALUE_USED(opline))
			{
				if (!p->key)
				{
	...
	else
	{
		...
		ZEND_VM_C_LABEL(fe_fetch_w_exit) : ZEND_VM_SET_RELATIVE_OPCODE(opline, opline->extended_value);
		ZEND_VM_CONTINUE();
	}
...
ZEND_VM_COLD_CONSTCONST_HANDLER(115, ZEND_ISSET_ISEMPTY_DIM_OBJ, CONST | TMPVAR | CV, CONST | TMPVAR | CV, ISSET)
{
	...
	if (EXPECTED(Z_TYPE_P(container) == IS_ARRAY))
	{
		...
		ZEND_VM_C_LABEL(isset_again) : if (EXPECTED(Z_TYPE_P(offset) == IS_STRING))
		{
			str = Z_STR_P(offset);
			if (OP2_TYPE != IS_CONST)
			{
...
ZEND_VM_HANDLER(58, ZEND_END_SILENCE, TMP, ANY)
{
	...
	if (E_HAS_ONLY_FATAL_ERRORS(EG(error_reporting)) && !E_HAS_ONLY_FATAL_ERRORS(Z_LVAL_P(EX_VAR(opline->op1.var))))
	{
...
ZEND_VM_HOT_NOCONST_HANDLER(198, ZEND_JMP_NULL, CONST | TMP | VAR | CV, JMP_ADDR)
{
	...
	{
		ZVAL_NULL(result);
		if (OP1_TYPE == IS_CV && UNEXPECTED(Z_TYPE_P(val) == IS_UNDEF) && (opline->extended_value & ZEND_JMP_NULL_BP_VAR_IS) == 0)
		{
			SAVE_OPLINE();
			ZVAL_UNDEFINED_OP1();
			if (UNEXPECTED(EG(exception) != NULL))
...
ZEND_VM_HELPER(zend_dispatch_try_catch_finally_helper, ANY, ANY, uint32_t try_catch_offset, uint32_t op_num)
{
	...
	for (; try_catch_offset != (uint32_t)-1; try_catch_offset--)
	{
		...
		else if (op_num < try_catch->finally_end)
		{
			...
			if (Z_OPLINE_NUM_P(fast_call) != (uint32_t)-1 && (EX(func)->op_array.opcodes[Z_OPLINE_NUM_P(fast_call)].op2_type & (IS_TMP_VAR | IS_VAR)))
			{
...
ZEND_VM_HANDLER(149, ZEND_HANDLE_EXCEPTION, ANY, ANY)
{
	...
	if ((throw_op->opcode == ZEND_FREE || throw_op->opcode == ZEND_FE_FREE) && throw_op->extended_value & ZEND_FREE_ON_RETURN)
	{
		...
		{
			if (EX(func)->op_array.opcodes[i].opcode == ZEND_FREE || EX(func)->op_array.opcodes[i].opcode == ZEND_FE_FREE)
			{
				/* pass */
			}
			else
			{
				if (EX(func)->op_array.opcodes[i].opcode == ZEND_RETURN && (EX(func)->op_array.opcodes[i].op1_type & (IS_VAR | IS_TMP_VAR)))
				{
...
ZEND_VM_HANDLER(160, ZEND_YIELD, CONST | TMP | VAR | CV | UNUSED, CONST | TMPVAR | CV | UNUSED, SRC)
{
			...
			else
			{
				...
				do
				{
					if (OP1_TYPE == IS_VAR)
					{
						...
						if (opline->extended_value == ZEND_RETURNS_FUNCTION && !Z_ISREF_P(value_ptr))
						{
							zend_error(E_NOTICE, "Only variable references should be yielded by reference");
	...
	if (OP2_TYPE != IS_UNUSED)
	{
		...
		if (Z_TYPE(generator->key) == IS_LONG && Z_LVAL(generator->key) > generator->largest_used_integer_key)
		{
...
ZEND_VM_HANDLER(166, ZEND_YIELD_FROM, CONST | TMPVAR | CV, ANY)
{
	...
	ZEND_VM_C_LABEL(yield_from_try_again) : if (Z_TYPE_P(val) == IS_ARRAY)
	{
...
ZEND_VM_HOT_HANDLER(168, ZEND_BIND_GLOBAL, CV, CONST, CACHE_SLOT)
{
	...
	else
	{
		...
		/* Store "hash slot index" + 1 (NULL is a mark of uninitialized cache slot) */
		CACHE_PTR(opline->extended_value, (void *)(idx + 1));
		ZEND_VM_C_LABEL(check_indirect) : /* GLOBAL variable may be an INDIRECT pointer to CV */
...
ZEND_VM_HOT_NOCONST_HANDLER(123, ZEND_TYPE_CHECK, CONST | TMPVAR | CV, ANY, TYPE_MASK)
{
	...
	{
		ZEND_VM_C_LABEL(type_check_resource) : if (opline->extended_value != MAY_BE_RESOURCE || EXPECTED(NULL != zend_rsrc_list_get_rsrc_type(Z_RES_P(value))))
		{
			result = 1;
		}
	}
...
ZEND_VM_HANDLER(157, ZEND_FETCH_CLASS_NAME, CV | TMPVAR | UNUSED | CLASS_FETCH, ANY)
{
	...
	if (UNEXPECTED(scope == NULL))
	{
		...
		zend_throw_error(NULL, "Cannot use \"%s\" in the global scope",
						 fetch_type == ZEND_FETCH_CLASS_SELF ? "self" : fetch_type == ZEND_FETCH_CLASS_PARENT ? "parent"
																											  : "static");
		ZVAL_UNDEF(EX_VAR(opline->result.var));
		HANDLE_EXCEPTION();
	}
...
ZEND_VM_COLD_CONSTCONST_HANDLER(195, ZEND_MATCH, CONST | TMPVARCV, CONST, JMP_ADDR)
{
	...
	ZEND_VM_C_LABEL(match_try_again) : if (Z_TYPE_P(op) == IS_LONG)
	{
		jump_zv = zend_hash_index_find(jumptable, Z_LVAL_P(op));
	}
	...
	{
		ZEND_VM_C_LABEL(default_branch) : /* default */
										  ZEND_VM_SET_RELATIVE_OPCODE(opline, opline->extended_value);
		ZEND_VM_CONTINUE();
	}
}
...
ZEND_VM_HANDLER(208, ZEND_JMP_FRAMELESS, CONST, JMP_ADDR, NUM | CACHE_SLOT)
{
	...
	ZEND_VM_C_LABEL(try_again) : if (EXPECTED(result == ZEND_JMP_FL_HIT))
	{
		OPLINE = OP_JMP_ADDR(opline, opline->op2);
		ZEND_VM_CONTINUE();
	}
...
ZEND_VM_HOT_TYPE_SPEC_HANDLER(ZEND_FETCH_DIM_R, (!(op2_info & (MAY_BE_UNDEF | MAY_BE_NULL | MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT | MAY_BE_RESOURCE | MAY_BE_REF))), ZEND_FETCH_DIM_R_INDEX, CONST | TMPVAR | CV, CONST | TMPVARCV, SPEC(NO_CONST_CONST))
{
	...
	if (EXPECTED(Z_TYPE_P(container) == IS_ARRAY))
	{
		ZEND_VM_C_LABEL(fetch_dim_r_index_array) : if (EXPECTED(Z_TYPE_P(dim) == IS_LONG))
		{
			offset = Z_LVAL_P(dim);
		}
		else
	...
	ZEND_VM_C_LABEL(fetch_dim_r_index_undef) : ZVAL_NULL(EX_VAR(opline->result.var));
	SAVE_OPLINE();
...
ZEND_VM_HELPER(zend_interrupt_helper, ANY, ANY)
{
	...
	else if (zend_interrupt_function)
	{
		...
		if (EG(exception))
		{
			...
			if (throw_op && throw_op->result_type & (IS_TMP_VAR | IS_VAR) && throw_op->opcode != ZEND_ADD_ARRAY_ELEMENT && throw_op->opcode != ZEND_ADD_ARRAY_UNPACK && throw_op->opcode != ZEND_ROPE_INIT && throw_op->opcode != ZEND_ROPE_ADD)
			{
