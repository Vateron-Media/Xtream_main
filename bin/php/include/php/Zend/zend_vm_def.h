...
ZEND_VM_COLD_CONSTCONST_HANDLER(6, ZEND_SL, CONST | TMPVAR | CV, CONST | TMPVAR | CV)
{
	...
	op1 = GET_OP1_ZVAL_PTR_UNDEF(BP_VAR_R);
	op2 = GET_OP2_ZVAL_PTR_UNDEF(BP_VAR_R);
	if (EXPECTED(Z_TYPE_INFO_P(op1) == IS_LONG) && EXPECTED(Z_TYPE_INFO_P(op2) == IS_LONG) && EXPECTED((zend_ulong)Z_LVAL_P(op2) < SIZEOF_ZEND_LONG * 8))
	{
...
ZEND_VM_HELPER(zend_binary_assign_op_obj_helper, VAR | UNUSED | CV, CONST | TMPVAR | CV, binary_op_type binary_op)
{
	...
	do
	{
		...
		ZEND_VM_C_LABEL(assign_op_object) : if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr) && EXPECTED((zptr = Z_OBJ_HT_P(object)->get_property_ptr_ptr(object, property, BP_VAR_RW, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR((opline + 1)->extended_value) : NULL))) != NULL))
		{
...
ZEND_VM_HELPER(zend_binary_assign_op_dim_helper, VAR | CV, CONST | TMPVAR | UNUSED | CV, binary_op_type binary_op)
{
	...
	else
	{
		...
		else if (OP1_TYPE == IS_CV && UNEXPECTED(Z_TYPE_INFO_P(container) == IS_UNDEF))
		{
			...
			ZEND_VM_C_LABEL(assign_dim_op_convert_to_array) : ZVAL_ARR(container, zend_new_array(8));
		...
		else
		{
			...
			else
			{
				...
				ZEND_VM_C_LABEL(assign_dim_op_ret_null) : if (UNEXPECTED(RETURN_VALUE_USED(opline)))
				{
					ZVAL_NULL(EX_VAR(opline->result.var));
				}
			}
...
ZEND_VM_HELPER(zend_pre_incdec_property_helper, VAR | UNUSED | CV, CONST | TMPVAR | CV, int inc)
{
	...
	do
	{
		...
		/* here we are sure we are dealing with an object */
		ZEND_VM_C_LABEL(pre_incdec_object) : if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr) && EXPECTED((zptr = Z_OBJ_HT_P(object)->get_property_ptr_ptr(object, property, BP_VAR_RW, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->extended_value) : NULL))) != NULL))
		{
			if (UNEXPECTED(Z_ISERROR_P(zptr)))
			{
				if (UNEXPECTED(RETURN_VALUE_USED(opline)))
...
ZEND_VM_HELPER(zend_post_incdec_property_helper, VAR | UNUSED | CV, CONST | TMPVAR | CV, int inc)
{
	...
	do
	{
		...
		/* here we are sure we are dealing with an object */
		ZEND_VM_C_LABEL(post_incdec_object) : if (EXPECTED(Z_OBJ_HT_P(object)->get_property_ptr_ptr) && EXPECTED((zptr = Z_OBJ_HT_P(object)->get_property_ptr_ptr(object, property, BP_VAR_RW, ((OP2_TYPE == IS_CONST) ? CACHE_ADDR(opline->extended_value) : NULL))) != NULL))
		{
			if (UNEXPECTED(Z_ISERROR_P(zptr)))
			{
				ZVAL_NULL(EX_VAR(opline->result.var));
...
ZEND_VM_HOT_OBJ_HANDLER(82, ZEND_FETCH_OBJ_R, CONST | TMPVAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, CACHE_SLOT)
{
	...
	do
	{
		...
		{
			ZEND_VM_C_LABEL(fetch_obj_r_no_object) : zend_wrong_property_read(offset);
			ZVAL_NULL(EX_VAR(opline->result.var));
		}
		else
		{
...
ZEND_VM_COLD_CONST_HANDLER(91, ZEND_FETCH_OBJ_IS, CONST | TMPVAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, CACHE_SLOT)
{
	...
	do
	{
		...
		if (UNEXPECTED(zobj->handlers->read_property == NULL))
		{
			ZEND_VM_C_LABEL(fetch_obj_is_no_object) : ZVAL_NULL(EX_VAR(opline->result.var));
		}
		else
		{
...
ZEND_VM_HANDLER(136, ZEND_ASSIGN_OBJ, VAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, CACHE_SLOT, SPEC(OP_DATA = CONST | TMP | VAR | CV))
{
	...
	ZEND_VM_C_LABEL(assign_object) : if (OP2_TYPE == IS_CONST &&
										 EXPECTED(Z_OBJCE_P(object) == CACHED_PTR(opline->extended_value)))
	{
		...
		if (EXPECTED(IS_VALID_PROPERTY_OFFSET(prop_offset)))
		{
			...
			{
				ZEND_VM_C_LABEL(fast_assign_obj) : value = zend_assign_to_variable(property_val, value, OP_DATA_TYPE);
				if (UNEXPECTED(RETURN_VALUE_USED(opline)))
				{
...
ZEND_VM_HOT_HANDLER(131, ZEND_DO_FCALL_BY_NAME, ANY, ANY, SPEC(RETVAL))
{
	...
	else
	{
		...
		if (UNEXPECTED(fbc->common.fn_flags & ZEND_ACC_HAS_TYPE_HINTS) && UNEXPECTED(!zend_verify_internal_arg_types(fbc, call)))
		{
			zend_vm_stack_free_call_frame(call);
			zend_rethrow_exception(execute_data);
			UNDEF_RESULT();
...
ZEND_VM_HOT_HANDLER(60, ZEND_DO_FCALL, ANY, ANY, SPEC(RETVAL))
{
	...
	ZEND_VM_C_LABEL(fcall_end) : if (UNEXPECTED(ZEND_CALL_INFO(call) & ZEND_CALL_RELEASE_THIS))
	{
...
/* prevents "undefined variable opline" errors */
		...
		if (UNEXPECTED(!ZEND_TYPE_IS_CLASS(ret_info->type) && ZEND_TYPE_CODE(ret_info->type) != IS_CALLABLE && ZEND_TYPE_CODE(ret_info->type) != IS_ITERABLE && !ZEND_SAME_FAKE_TYPE(ZEND_TYPE_CODE(ret_info->type), Z_TYPE_P(retval_ptr)) && !(EX(func)->op_array.fn_flags & ZEND_ACC_RETURN_REFERENCE) && retval_ref != retval_ptr))
		{
...
ZEND_VM_HANDLER(41, ZEND_GENERATOR_CREATE, ANY, ANY)
{
	...
	if (EXPECTED(return_value))
	{
		...
		if ((call_info & Z_TYPE_MASK) == IS_OBJECT && (!(call_info & ((ZEND_CALL_CLOSURE | ZEND_CALL_RELEASE_THIS) << ZEND_CALL_INFO_SHIFT))
													   /* Bug #72523 */
...
ZEND_VM_HOT_SEND_HANDLER(116, ZEND_SEND_VAL_EX, CONST | TMP, NUM, SPEC(QUICK_ARG))
{
	...
	{
		ZEND_VM_C_LABEL(send_val_by_ref) : ZEND_VM_DISPATCH_TO_HELPER(zend_cannot_pass_by_ref_helper);
	}
	value = GET_OP1_ZVAL_PTR(BP_VAR_R);
	arg = ZEND_CALL_VAR(EX(call), opline->result.var);
	ZVAL_COPY_VALUE(arg, value);
...
ZEND_VM_HOT_SEND_HANDLER(66, ZEND_SEND_VAR_EX, VAR | CV, NUM, SPEC(QUICK_ARG))
{
	...
	{
		ZEND_VM_C_LABEL(send_var_by_ref) : ZEND_VM_DISPATCH_TO_HANDLER(ZEND_SEND_REF);
	}
				...
				if (iter->funcs->get_current_key)
				{
					...
					if (UNEXPECTED(Z_TYPE(key) != IS_LONG))
					{
						zend_throw_error(NULL,
										 (Z_TYPE(key) == IS_STRING) ? "Cannot unpack Traversable with string keys" : "Cannot unpack Traversable with non-integer keys");
						zval_ptr_dtor(&key);
						break;
					}
				}
			...
			while (1)
			{
				...
				if (EXPECTED(value_type != IS_UNDEF))
				{
					if (UNEXPECTED(value_type == IS_INDIRECT))
					{
						value = Z_INDIRECT_P(value);
						value_type = Z_TYPE_INFO_P(value);
						if (EXPECTED(value_type != IS_UNDEF) && EXPECTED(zend_check_property_access(Z_OBJ_P(array), p->key) == SUCCESS))
						{
							break;
						}
					}
...
ZEND_VM_HANDLER(180, ZEND_ISSET_ISEMPTY_STATIC_PROP, CONST | TMPVAR | CV, UNUSED | CLASS_FETCH | CONST | VAR, ISSET | CACHE_SLOT)
{
	...
	ZEND_VM_C_LABEL(is_static_prop_return) : if (!(opline->extended_value & ZEND_ISEMPTY))
	{
...
ZEND_VM_COLD_CONST_HANDLER(148, ZEND_ISSET_ISEMPTY_PROP_OBJ, CONST | TMPVAR | UNUSED | THIS | CV, CONST | TMPVAR | CV, ISSET | CACHE_SLOT)
{
	...
	if (UNEXPECTED(!Z_OBJ_HT_P(container)->has_property))
	{
		...
		ZEND_VM_C_LABEL(isset_no_object) : result = (opline->extended_value & ZEND_ISEMPTY);
	}
	else
	{
		result =
...
ZEND_VM_HOT_NOCONST_HANDLER(123, ZEND_TYPE_CHECK, CONST | TMPVAR | CV, ANY, TYPE_MASK)
{
	...
	{
		ZEND_VM_C_LABEL(type_check_resource) : if (EXPECTED(Z_TYPE_P(value) != IS_RESOURCE) || EXPECTED(NULL != zend_rsrc_list_get_rsrc_type(Z_RES_P(value))))
		{
			result = 1;
		}
	}
...
ZEND_VM_HANDLER(158, ZEND_CALL_TRAMPOLINE, ANY, ANY)
{
	...
	ZEND_VM_C_LABEL(call_trampoline_end) : execute_data = EG(current_execute_data);
