...
#define ZEND_INI_MH(name)                                                      \
  int name(zend_ini_entry *entry, zend_string *new_value, void *mh_arg1,       \
...
ZEND_API int zend_alter_ini_entry(zend_string *name, zend_string *new_value,
                                  int modify_type, int stage);
ZEND_API int zend_alter_ini_entry_ex(zend_string *name, zend_string *new_value,
                                     int modify_type, int stage,
                                     int force_change);
ZEND_API int zend_alter_ini_entry_chars(zend_string *name, const char *value,
                                        size_t value_length, int modify_type,
                                        int stage);
ZEND_API int zend_alter_ini_entry_chars_ex(zend_string *name, const char *value,
                                           size_t value_length, int modify_type,
                                           int stage, int force_change);
...
ZEND_API char *zend_ini_string_ex(char *name, size_t name_length, int orig,
                                  zend_bool *exists);
ZEND_API zend_string *zend_ini_get_value(zend_string *name);
ZEND_API zend_bool zend_ini_parse_bool(zend_string *str);

ZEND_API int zend_ini_register_displayer(
    char *name, uint32_t name_length,
    void (*displayer)(zend_ini_entry *ini_entry, int type));
...
#define ZEND_INI_ENTRY3_EX(name, default_value, modifiable, on_modify, arg1,   \
                           arg2, arg3, displayer)                              \
  {name,                                                                       \
   on_modify,                                                                  \
   arg1,                                                                       \
   ...
   default_value,                                                              \
   displayer,                                                                  \
   sizeof(default_value) - 1,                                                  \
   sizeof(name) - 1,                                                           \
   modifiable},

#define ZEND_INI_ENTRY3(name, default_value, modifiable, on_modify, arg1,      \
                        arg2, arg3)                                            \
  ZEND_INI_ENTRY3_EX(name, default_value, modifiable, on_modify, arg1, arg2,   \
                     arg3, NULL)

#define ZEND_INI_ENTRY2_EX(name, default_value, modifiable, on_modify, arg1,   \
                           arg2, displayer)                                    \
  ZEND_INI_ENTRY3_EX(name, default_value, modifiable, on_modify, arg1, arg2,   \
                     NULL, displayer)

#define ZEND_INI_ENTRY2(name, default_value, modifiable, on_modify, arg1,      \
                        arg2)                                                  \
  ZEND_INI_ENTRY2_EX(name, default_value, modifiable, on_modify, arg1, arg2,   \
                     NULL)

#define ZEND_INI_ENTRY1_EX(name, default_value, modifiable, on_modify, arg1,   \
                           displayer)                                          \
  ZEND_INI_ENTRY3_EX(name, default_value, modifiable, on_modify, arg1, NULL,   \
                     NULL, displayer)
...
#ifdef ZTS
#define STD_ZEND_INI_ENTRY(name, default_value, modifiable, on_modify,         \
                           property_name, struct_type, struct_ptr)             \
  ZEND_INI_ENTRY2(name, default_value, modifiable, on_modify,                  \
                  (void *)XtOffsetOf(struct_type, property_name),              \
                  (void *)&struct_ptr##_id)
#define STD_ZEND_INI_ENTRY_EX(name, default_value, modifiable, on_modify,      \
                              property_name, struct_type, struct_ptr,          \
                              displayer)                                       \
  ZEND_INI_ENTRY2_EX(name, default_value, modifiable, on_modify,               \
                     (void *)XtOffsetOf(struct_type, property_name),           \
                     (void *)&struct_ptr##_id, displayer)
#define STD_ZEND_INI_BOOLEAN(name, default_value, modifiable, on_modify,       \
                             property_name, struct_type, struct_ptr)           \
  ZEND_INI_ENTRY3_EX(name, default_value, modifiable, on_modify,               \
                     (void *)XtOffsetOf(struct_type, property_name),           \
                     (void *)&struct_ptr##_id, NULL,                           \
...
#define STD_ZEND_INI_ENTRY_EX(name, default_value, modifiable, on_modify,      \
  ...
  ZEND_INI_ENTRY2_EX(name, default_value, modifiable, on_modify,               \
                     (void *)XtOffsetOf(struct_type, property_name),           \
                     (void *)&struct_ptr, displayer)
#define STD_ZEND_INI_BOOLEAN(name, default_value, modifiable, on_modify,       \
                             property_name, struct_type, struct_ptr)           \
  ZEND_INI_ENTRY3_EX(name, default_value, modifiable, on_modify,               \
                     (void *)XtOffsetOf(struct_type, property_name),           \
                     (void *)&struct_ptr, NULL, zend_ini_boolean_displayer_cb)
#endif
...
#define REGISTER_INI_DISPLAYER(name, displayer)                                \
  zend_ini_register_displayer((name), sizeof(name) - 1, displayer)
...
#define ZEND_INI_STAGE_IN_REQUEST                                              \
  (ZEND_INI_STAGE_ACTIVATE | ZEND_INI_STAGE_DEACTIVATE |                       \
   ZEND_INI_STAGE_RUNTIME | ZEND_INI_STAGE_HTACCESS)
...
ZEND_API int zend_parse_ini_file(zend_file_handle *fh,
                                 zend_bool unbuffered_errors, int scanner_mode,
