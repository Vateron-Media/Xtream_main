...
ZEND_API char *ZEND_FASTCALL zend_strndup(const char *s,
                                          size_t length) ZEND_ATTRIBUTE_MALLOC;

ZEND_API void *ZEND_FASTCALL _emalloc(
    size_t size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC) ZEND_ATTRIBUTE_MALLOC
    ZEND_ATTRIBUTE_ALLOC_SIZE(1);
...
ZEND_API void *ZEND_FASTCALL _safe_malloc(size_t nmemb, size_t size,
                                          size_t offset) ZEND_ATTRIBUTE_MALLOC;
ZEND_API void ZEND_FASTCALL
_efree(void *ptr ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
ZEND_API void *ZEND_FASTCALL _ecalloc(
    size_t nmemb,
    size_t size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC) ZEND_ATTRIBUTE_MALLOC
    ZEND_ATTRIBUTE_ALLOC_SIZE2(1, 2);
ZEND_API void *ZEND_FASTCALL
_erealloc(void *ptr, size_t size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC)
    ZEND_ATTRIBUTE_ALLOC_SIZE(2);
ZEND_API void *ZEND_FASTCALL
_erealloc2(void *ptr, size_t size,
           size_t copy_size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC)
    ZEND_ATTRIBUTE_ALLOC_SIZE(2);
ZEND_API void *ZEND_FASTCALL
_safe_erealloc(void *ptr, size_t nmemb, size_t size,
               size_t offset ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
ZEND_API void *ZEND_FASTCALL _safe_realloc(void *ptr, size_t nmemb, size_t size,
                                           size_t offset);
ZEND_API char *ZEND_FASTCALL
_estrdup(const char *s ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC)
    ZEND_ATTRIBUTE_MALLOC;
ZEND_API char *ZEND_FASTCALL
_estrndup(const char *s, size_t length ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC)
    ZEND_ATTRIBUTE_MALLOC;
ZEND_API size_t ZEND_FASTCALL
_zend_mem_block_size(void *ptr ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
...
ZEND_API void *ZEND_FASTCALL _emalloc_large(size_t size) ZEND_ATTRIBUTE_MALLOC
    ZEND_ATTRIBUTE_ALLOC_SIZE(1);
ZEND_API void *ZEND_FASTCALL _emalloc_huge(size_t size) ZEND_ATTRIBUTE_MALLOC
    ZEND_ATTRIBUTE_ALLOC_SIZE(1);

#define _ZEND_BIN_ALLOCATOR_SELECTOR_START(_num, _size, _elements, _pages,     \
                                           size, y)                            \
        ((size <= _size) ? _emalloc_ ## _size() :
#define _ZEND_BIN_ALLOCATOR_SELECTOR_END(_num, _size, _elements, _pages, size, \
                                         y)                                    \
        )
...
#define _emalloc(size)                                                         \
  (__builtin_constant_p(size) ? ZEND_ALLOCATOR(size) : _emalloc(size))
...
#define _ZEND_BIN_DEALLOCATOR_SELECTOR_START(_num, _size, _elements, _pages,   \
                                             ptr, size)                        \
  if (size <= _size) {                                                         \
    _efree_##_size(ptr);                                                       \
  } else
...
#define ZEND_DEALLOCATOR(ptr, size)                                            \
  ...
  if (size <= ZEND_MM_MAX_LARGE_SIZE) {                                        \
    _efree_large(ptr, size);                                                   \
  } else {                                                                     \
    _efree_huge(ptr, size);                                                    \
  }
...
#define efree_size_rel(ptr, size) efree_size(ptr, size)
...
#define emalloc_large(size)                                                    \
  _emalloc_large((size)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define emalloc_huge(size)                                                     \
  _emalloc_huge((size)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define safe_emalloc(nmemb, size, offset)                                      \
  _safe_emalloc((nmemb), (size),                                               \
                (offset)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define efree(ptr) _efree((ptr)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define efree_large(ptr)                                                       \
  _efree_large((ptr)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define efree_huge(ptr)                                                        \
  _efree_huge((ptr)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define ecalloc(nmemb, size)                                                   \
  _ecalloc((nmemb), (size)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define erealloc(ptr, size)                                                    \
  _erealloc((ptr), (size)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define erealloc2(ptr, size, copy_size)                                        \
  _erealloc2((ptr), (size),                                                    \
             (copy_size)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
#define safe_erealloc(ptr, nmemb, size, offset)                                \
  _safe_erealloc((ptr), (nmemb), (size),                                       \
                 (offset)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)
...
#define zend_mem_block_size(ptr)                                               \
  _zend_mem_block_size((ptr)ZEND_FILE_LINE_CC ZEND_FILE_LINE_EMPTY_CC)

/* Relay wrapper macros */
#define emalloc_rel(size)                                                      \
  _emalloc((size)ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_CC)
#define safe_emalloc_rel(nmemb, size, offset)                                  \
  _safe_emalloc((nmemb), (size),                                               \
                (offset)ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_CC)
...
#define erealloc_rel(ptr, size)                                                \
  _erealloc((ptr), (size)ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_CC)
#define erealloc2_rel(ptr, size, copy_size)                                    \
  _erealloc2((ptr), (size),                                                    \
             (copy_size)ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_CC)
...
#define zend_mem_block_size_rel(ptr)                                           \
  _zend_mem_block_size((ptr)ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_CC)

ZEND_API void *__zend_malloc(size_t len) ZEND_ATTRIBUTE_MALLOC
    ZEND_ATTRIBUTE_ALLOC_SIZE(1);
ZEND_API void *__zend_calloc(size_t nmemb, size_t len) ZEND_ATTRIBUTE_MALLOC
    ZEND_ATTRIBUTE_ALLOC_SIZE2(1, 2);
...
#define safe_pemalloc(nmemb, size, offset, persistent)                         \
  ((persistent) ? _safe_malloc(nmemb, size, offset)                            \
...
#define perealloc2(ptr, size, copy_size, persistent)                           \
  ((persistent) ? __zend_realloc((ptr), (size))                                \
                : erealloc2((ptr), (size), (copy_size)))
#define safe_perealloc(ptr, nmemb, size, offset, persistent)                   \
  ((persistent) ? _safe_realloc((ptr), (nmemb), (size), (offset))              \
                : safe_erealloc((ptr), (nmemb), (size), (offset)))
#define perealloc_recoverable(ptr, size, persistent)                           \
  ((persistent) ? realloc((ptr), (size)) : erealloc_recoverable((ptr), (size)))
#define perealloc2_recoverable(ptr, size, persistent)                          \
  ((persistent) ? realloc((ptr), (size))                                       \
                : erealloc2_recoverable((ptr), (size), (copy_size)))
...
#define perealloc2_rel(ptr, size, copy_size, persistent)                       \
  ((persistent) ? __zend_realloc((ptr), (size))                                \
                : erealloc2_rel((ptr), (size), (copy_size)))
...
#define perealloc2_recoverable_rel(ptr, size, copy_size, persistent)           \
  ((persistent) ? realloc((ptr), (size))                                       \
                : erealloc2_recoverable_rel((ptr), (size), (copy_size)))
...
ZEND_API void *ZEND_FASTCALL _zend_mm_alloc(
    zend_mm_heap *heap,
    size_t size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC) ZEND_ATTRIBUTE_MALLOC;
ZEND_API void ZEND_FASTCALL _zend_mm_free(
    zend_mm_heap *heap, void *p ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
ZEND_API void *ZEND_FASTCALL
_zend_mm_realloc(zend_mm_heap *heap, void *p,
                 size_t size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
ZEND_API void *ZEND_FASTCALL
_zend_mm_realloc2(zend_mm_heap *heap, void *p, size_t size,
                  size_t copy_size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
ZEND_API size_t ZEND_FASTCALL _zend_mm_block_size(
    zend_mm_heap *heap, void *p ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC);
...
#if ZEND_DEBUG
ZEND_API void zend_mm_set_custom_debug_handlers(
    zend_mm_heap *heap,
...
typedef void (*zend_mm_chunk_free_t)(zend_mm_storage *storage, void *chunk,
                                     size_t size);
typedef int (*zend_mm_chunk_truncate_t)(zend_mm_storage *storage, void *chunk,
                                        size_t old_size, size_t new_size);
typedef int (*zend_mm_chunk_extend_t)(zend_mm_storage *storage, void *chunk,
                                      size_t old_size, size_t new_size);
...
static void *apc_chunk_alloc(zend_mm_storage *storage, size_t size, size_t
alignment)
{
        apc_data *data = (apc_data*)(storage->data);
        size_t real_size = ((size + (ZEND_MM_CHUNK_SIZE-1)) &
~(ZEND_MM_CHUNK_SIZE-1)); uint32_t count = real_size / ZEND_MM_CHUNK_SIZE;
                ...
                if (!(data->free_pages & (1 << first))) {
                        ...
                        do {
                                if (last - first == count - 1) {
                                        ...
                                        return (void *)(((char*)(data->mem)) +
ZEND_MM_CHUNK_SIZE * (1 << first));
                                }
                                last++;
                        } while (last < 32 && !(data->free_pages & (1 <<
last))); first = last;
                }
        }
        return NULL;
...
static void apc_init_heap(void)
{
        ...
        tmp_data.mem = shm_memalign(ZEND_MM_CHUNK_SIZE, ZEND_MM_CHUNK_SIZE *
32);
        ...
        // Create heap
        apc_heap = zend_mm_startup_ex(&apc_handlers, &tmp_data,
sizeof(tmp_data));
