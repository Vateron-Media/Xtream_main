...
ZEND_API int ZEND_FASTCALL boolean_xor_function(zval *result, zval *op1,
                                                zval *op2);
...
ZEND_API int ZEND_FASTCALL bitwise_or_function(zval *result, zval *op1,
                                               zval *op2);
ZEND_API int ZEND_FASTCALL bitwise_and_function(zval *result, zval *op1,
                                                zval *op2);
ZEND_API int ZEND_FASTCALL bitwise_xor_function(zval *result, zval *op1,
                                                zval *op2);
ZEND_API int ZEND_FASTCALL shift_left_function(zval *result, zval *op1,
                                               zval *op2);
ZEND_API int ZEND_FASTCALL shift_right_function(zval *result, zval *op1,
                                                zval *op2);
...
ZEND_API int ZEND_FASTCALL is_equal_function(zval *result, zval *op1,
                                             zval *op2);
ZEND_API int ZEND_FASTCALL is_identical_function(zval *result, zval *op1,
                                                 zval *op2);
ZEND_API int ZEND_FASTCALL is_not_identical_function(zval *result, zval *op1,
                                                     zval *op2);
ZEND_API int ZEND_FASTCALL is_not_equal_function(zval *result, zval *op1,
                                                 zval *op2);
ZEND_API int ZEND_FASTCALL is_smaller_function(zval *result, zval *op1,
                                               zval *op2);
ZEND_API int ZEND_FASTCALL is_smaller_or_equal_function(zval *result, zval *op1,
                                                        zval *op2);

ZEND_API zend_bool ZEND_FASTCALL
instanceof_function_ex(const zend_class_entry *instance_ce,
                       const zend_class_entry *ce, zend_bool is_interface);
ZEND_API zend_bool ZEND_FASTCALL instanceof_function(
 ...
 * if the integer is larger than ZEND_LONG_MAX and -1 if it's smaller than
 * ZEND_LONG_MIN.
 */
...
static zend_always_inline zend_uchar is_numeric_string(const char *str,
                                                       size_t length,
                                                       zend_long *lval,
                                                       double *dval,
                                                       int allow_errors) {
...
static zend_always_inline const char *zend_memnstr(const char *haystack,
                                                   const char *needle,
                                                   size_t needle_len,
                                                   const char *end) {
  ...
  if (EXPECTED(off_s < 1024 ||
               needle_len <
                   9)) { /* glibc memchr is faster when needle is too short */
    end -= needle_len;
...
static zend_always_inline const char *zend_memnrstr(const char *haystack,
                                                    const char *needle,
                                                    size_t needle_len,
                                                    const char *end) {
  const char *p = end;
...
static zend_always_inline zend_long zval_get_long(zval *op) {
  return EXPECTED(Z_TYPE_P(op) == IS_LONG) ? Z_LVAL_P(op)
                                           : zval_get_long_func(op);
}
static zend_always_inline double zval_get_double(zval *op) {
  return EXPECTED(Z_TYPE_P(op) == IS_DOUBLE) ? Z_DVAL_P(op)
                                             : zval_get_double_func(op);
}
static zend_always_inline zend_string *zval_get_string(zval *op) {
  return EXPECTED(Z_TYPE_P(op) == IS_STRING) ? zend_string_copy(Z_STR_P(op))
                                             : zval_get_string_func(op);
}

static zend_always_inline zend_string *zval_get_tmp_string(zval *op,
                                                           zend_string **tmp) {
...
/* Like zval_get_string, but returns NULL if the conversion fails with an
 * exception. */
...
/* Like zval_get_tmp_string, but returns NULL if the conversion fails with an
 * exception. */
static zend_always_inline zend_string *
zval_try_get_tmp_string(zval *op, zend_string **tmp) {
  if (EXPECTED(Z_TYPE_P(op) == IS_STRING)) {
...
/* Like convert_to_string(), but returns whether the conversion succeeded and
 * does not modify the zval in-place if it fails. */
...
#define convert_to_cstring(op)                                                 \
  if (Z_TYPE_P(op) != IS_STRING) {                                             \
    _convert_to_cstring((op));                                                 \
  }
...
again:
  ...
  case IS_ARRAY:
    ...
    break;
  case IS_OBJECT:
    if (EXPECTED(Z_OBJ_HT_P(op)->cast_object ==
...
ZEND_API int ZEND_FASTCALL
string_compare_function_ex(zval *op1, zval *op2, zend_bool case_insensitive);
...
ZEND_API int ZEND_FASTCALL zend_binary_zval_strncmp(zval *s1, zval *s2,
                                                    zval *s3);
ZEND_API int ZEND_FASTCALL zend_binary_zval_strcasecmp(zval *s1, zval *s2);
ZEND_API int ZEND_FASTCALL zend_binary_zval_strncasecmp(zval *s1, zval *s2,
                                                        zval *s3);
ZEND_API int ZEND_FASTCALL zend_binary_strcmp(const char *s1, size_t len1,
                                              const char *s2, size_t len2);
ZEND_API int ZEND_FASTCALL zend_binary_strncmp(const char *s1, size_t len1,
                                               const char *s2, size_t len2,
                                               size_t length);
ZEND_API int ZEND_FASTCALL zend_binary_strcasecmp(const char *s1, size_t len1,
                                                  const char *s2, size_t len2);
ZEND_API int ZEND_FASTCALL zend_binary_strncasecmp(const char *s1, size_t len1,
                                                   const char *s2, size_t len2,
                                                   size_t length);
ZEND_API int ZEND_FASTCALL zend_binary_strcasecmp_l(const char *s1, size_t len1,
                                                    const char *s2,
                                                    size_t len2);
ZEND_API int ZEND_FASTCALL zend_binary_strncasecmp_l(
    const char *s1, size_t len1, const char *s2, size_t len2, size_t length);
...
zend_locale_sprintf_double(zval *op ZEND_FILE_LINE_DC);
...
#define convert_to_boolean_ex(pzv)                                             \
  do {                                                                         \
...
/* This performance improvement of tolower() on Windows gives 10-18% on
 * bench.php */
...
static zend_always_inline void fast_long_add_function(zval *result, zval *op1,
                                                      zval *op2) {
#if ZEND_USE_ASM_ARITHMETIC && defined(__i386__) &&                            \
    !(4 == __GNUC__ && 8 == __GNUC_MINOR__)
  __asm__ goto("movl	(%1), %%eax\n\t"
               ...
               : "r"(&result->value), "r"(&op1->value), "r"(&op2->value),
                 "n"(IS_LONG), "n"(ZVAL_OFFSETOF_TYPE)
               : "eax", "cc", "memory"
               : overflow);
  return;
overflow:
...
overflow:
  ...
  long lresult;
  if (UNEXPECTED(
          __builtin_saddl_overflow(Z_LVAL_P(op1), Z_LVAL_P(op2), &lresult))) {
    ZVAL_DOUBLE(result, (double)Z_LVAL_P(op1) + (double)Z_LVAL_P(op2));
  } else {
  ...
  long long llresult;
  if (UNEXPECTED(
          __builtin_saddll_overflow(Z_LVAL_P(op1), Z_LVAL_P(op2), &llresult))) {
    ZVAL_DOUBLE(result, (double)Z_LVAL_P(op1) + (double)Z_LVAL_P(op2));
  } else {
...
static zend_always_inline int fast_add_function(zval *result, zval *op1,
                                                zval *op2) {
...
static zend_always_inline void fast_long_sub_function(zval *result, zval *op1,
                                                      zval *op2) {
#if ZEND_USE_ASM_ARITHMETIC && defined(__i386__) &&                            \
    !(4 == __GNUC__ && 8 == __GNUC_MINOR__)
  __asm__ goto("movl	(%1), %%eax\n\t"
...
overflow:
  ...
  long lresult;
  if (UNEXPECTED(
          __builtin_ssubl_overflow(Z_LVAL_P(op1), Z_LVAL_P(op2), &lresult))) {
    ZVAL_DOUBLE(result, (double)Z_LVAL_P(op1) - (double)Z_LVAL_P(op2));
  } else {
    ZVAL_LONG(result, lresult);
  }
  ...
  long long llresult;
  if (UNEXPECTED(
          __builtin_ssubll_overflow(Z_LVAL_P(op1), Z_LVAL_P(op2), &llresult))) {
    ZVAL_DOUBLE(result, (double)Z_LVAL_P(op1) - (double)Z_LVAL_P(op2));
  } else {
    ZVAL_LONG(result, llresult);
  }
...
static zend_always_inline int fast_div_function(zval *result, zval *op1,
                                                zval *op2) {
  return div_function(result, op1, op2);
}

static zend_always_inline int zend_fast_equal_strings(zend_string *s1,
                                                      zend_string *s2) {
  if (s1 == s2) {
    return 1;
...
static zend_always_inline zend_bool fast_is_identical_function(zval *op1,
                                                               zval *op2) {
...
static zend_always_inline zend_bool fast_is_not_identical_function(zval *op1,
                                                                   zval *op2) {
  if (Z_TYPE_P(op1) != Z_TYPE_P(op2)) {
    return 1;
  } else if (Z_TYPE_P(op1) <= IS_TRUE) {
...
#define ZEND_TRY_BINARY_OP1_OBJECT_OPERATION(opcode, binary_op)                \
  if (UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result &&               \
    ...
    if (EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(opcode, result, \
                                                               op1, op2))) {   \
      return SUCCESS;                                                          \
    }                                                                          \
  }
...
#define ZEND_TRY_BINARY_OP2_OBJECT_OPERATION(opcode)                           \
  if (UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) &&                                \
      ...
      EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(opcode, result,   \
                                                             op1, op2))) {     \
    return SUCCESS;                                                            \
  }
...
#define ZEND_TRY_BINARY_OBJECT_OPERATION(opcode, binary_op)                    \
  ...
  else ZEND_TRY_BINARY_OP2_OBJECT_OPERATION(opcode)

#define ZEND_TRY_UNARY_OBJECT_OPERATION(opcode)                                \
  if (UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) &&                                \
      UNEXPECTED(Z_OBJ_HANDLER_P(op1, do_operation)) &&                        \
      EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(opcode, result,   \
                                                             op1, NULL))) {    \
    return SUCCESS;                                                            \
  }

/* buf points to the END of the buffer */
static zend_always_inline char *zend_print_ulong_to_buf(char *buf,
                                                        zend_ulong num) {
  *buf = '\0';
  do {
    *--buf = (char)(num % 10) + '0';
...
/* buf points to the END of the buffer */
static zend_always_inline char *zend_print_long_to_buf(char *buf,
                                                       zend_long num) {
  if (num < 0) {
