...
typedef zend_string *(ZEND_FASTCALL *zend_new_interned_string_func_t)(
    zend_string *str);
typedef zend_string *(ZEND_FASTCALL *zend_string_init_interned_func_t)(
    const char *str, size_t size, int permanent);
...
ZEND_API zend_string *ZEND_FASTCALL
...
#define ZSTR_ALLOCA_ALLOC(str, _len, use_heap)                                 \
  do {                                                                         \
    (str) = (zend_string *)do_alloca(                                          \
        ZEND_MM_ALIGNED_SIZE_EX(_ZSTR_STRUCT_SIZE(_len), 8), (use_heap));      \
...
static zend_always_inline zend_string *zend_string_alloc(size_t len,
                                                         int persistent) {
  zend_string *ret = (zend_string *)pemalloc(
      ZEND_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(len)), persistent);
...
zend_string_safe_alloc(size_t n, size_t m, size_t l, int persistent) {
  zend_string *ret = (zend_string *)safe_pemalloc(
      n, m, ZEND_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(l)), persistent);
...
static zend_always_inline zend_string *zend_string_dup(zend_string *s,
                                                       int persistent) {
  if (ZSTR_IS_INTERNED(s)) {
    return s;
  } else {
...
zend_string_realloc(zend_string *s, size_t len, int persistent) {
  ...
  if (!ZSTR_IS_INTERNED(s)) {
    if (EXPECTED(GC_REFCOUNT(s) == 1)) {
      ret = (zend_string *)perealloc(
          s, ZEND_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(len)), persistent);
      ZSTR_LEN(ret) = len;
...
static zend_always_inline zend_string *
zend_string_extend(zend_string *s, size_t len, int persistent) {
  zend_string *ret;
...
static zend_always_inline zend_string *
zend_string_truncate(zend_string *s, size_t len, int persistent) {
  zend_string *ret;
...
static zend_always_inline zend_string *
zend_string_safe_realloc(zend_string *s, size_t n, size_t m, size_t l,
                         int persistent) {
  zend_string *ret;

  if (!ZSTR_IS_INTERNED(s)) {
    if (GC_REFCOUNT(s) == 1) {
      ret = (zend_string *)safe_perealloc(
          s, n, m, ZEND_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(l)), persistent);
      ZSTR_LEN(ret) = (n * m) + l;
      zend_string_forget_hash_val(ret);
      return ret;
    }
...
static zend_always_inline void zend_string_release_ex(zend_string *s,
                                                      int persistent) {
  if (!ZSTR_IS_INTERNED(s)) {
    if (GC_DELREF(s) == 0) {
      if (persistent) {
...
BEGIN_EXTERN_C()
ZEND_API zend_bool ZEND_FASTCALL zend_string_equal_val(zend_string *s1,
                                                       zend_string *s2);
END_EXTERN_C()
#else
static zend_always_inline zend_bool zend_string_equal_val(zend_string *s1,
                                                          zend_string *s2) {
...
static zend_always_inline zend_bool zend_string_equals(zend_string *s1,
                                                       zend_string *s2) {
...
#define zend_string_equals_ci(s1, s2)                                          \
  (ZSTR_LEN(s1) == ZSTR_LEN(s2) &&                                             \
   !zend_binary_strcasecmp(ZSTR_VAL(s1), ZSTR_LEN(s1), ZSTR_VAL(s2),           \
                           ZSTR_LEN(s2)))

#define zend_string_equals_literal_ci(str, c)                                  \
  (ZSTR_LEN(str) == sizeof(c) - 1 &&                                           \
   !zend_binary_strcasecmp(ZSTR_VAL(str), ZSTR_LEN(str), (c), sizeof(c) - 1))

#define zend_string_equals_literal(str, literal)                               \
  (ZSTR_LEN(str) == sizeof(literal) - 1 &&                                     \
   !memcmp(ZSTR_VAL(str), literal, sizeof(literal) - 1))
...
static zend_always_inline zend_ulong zend_inline_hash_func(const char *str,
                                                           size_t len) {
...
#else
    hash = hash * 33 * 33 * 33 * 33 + str[0] * 33 * 33 * 33 + str[1] * 33 * 33 +
           str[2] * 33 + str[3];
    hash = hash * 33 * 33 * 33 * 33 + str[4] * 33 * 33 * 33 + str[5] * 33 * 33 +
           str[6] * 33 + str[7];
#endif
  }
  if (len >= 4) {
