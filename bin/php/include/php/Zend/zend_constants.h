...
/* Flag for zend_get_constant_ex(). Must not class with ZEND_FETCH_CLASS_*
 * flags. */
...
#define REGISTER_NS_NULL_CONSTANT(ns, name, flags)                             \
  zend_register_null_constant(ZEND_NS_NAME(ns, name),                          \
                              sizeof(ZEND_NS_NAME(ns, name)) - 1, (flags),     \
                              module_number)
#define REGISTER_NS_BOOL_CONSTANT(ns, name, bval, flags)                       \
  zend_register_bool_constant(ZEND_NS_NAME(ns, name),                          \
                              sizeof(ZEND_NS_NAME(ns, name)) - 1, (bval),      \
                              (flags), module_number)
#define REGISTER_NS_LONG_CONSTANT(ns, name, lval, flags)                       \
  zend_register_long_constant(ZEND_NS_NAME(ns, name),                          \
                              sizeof(ZEND_NS_NAME(ns, name)) - 1, (lval),      \
                              (flags), module_number)
#define REGISTER_NS_DOUBLE_CONSTANT(ns, name, dval, flags)                     \
  zend_register_double_constant(ZEND_NS_NAME(ns, name),                        \
                                sizeof(ZEND_NS_NAME(ns, name)) - 1, (dval),    \
                                (flags), module_number)
#define REGISTER_NS_STRING_CONSTANT(ns, name, str, flags)                      \
  zend_register_string_constant(ZEND_NS_NAME(ns, name),                        \
                                sizeof(ZEND_NS_NAME(ns, name)) - 1, (str),     \
                                (flags), module_number)
#define REGISTER_NS_STRINGL_CONSTANT(ns, name, str, len, flags)                \
  zend_register_stringl_constant(ZEND_NS_NAME(ns, name),                       \
                                 sizeof(ZEND_NS_NAME(ns, name)) - 1, (str),    \
                                 (len), (flags), module_number)

#define REGISTER_MAIN_NULL_CONSTANT(name, flags)                               \
  zend_register_null_constant((name), sizeof(name) - 1, (flags), 0)
...
#define REGISTER_MAIN_DOUBLE_CONSTANT(name, dval, flags)                       \
  zend_register_double_constant((name), sizeof(name) - 1, (dval), (flags), 0)
...
ZEND_API int zend_verify_const_access(zend_class_constant *c,
                                      zend_class_entry *ce);
ZEND_API zval *zend_get_constant(zend_string *name);
ZEND_API zval *zend_get_constant_str(const char *name, size_t name_len);
ZEND_API zval *zend_get_constant_ex(zend_string *name, zend_class_entry *scope,
                                    uint32_t flags);
ZEND_API void zend_register_bool_constant(const char *name, size_t name_len,
                                          zend_bool bval, int flags,
                                          int module_number);
ZEND_API void zend_register_null_constant(const char *name, size_t name_len,
                                          int flags, int module_number);
ZEND_API void zend_register_long_constant(const char *name, size_t name_len,
                                          zend_long lval, int flags,
                                          int module_number);
ZEND_API void zend_register_double_constant(const char *name, size_t name_len,
                                            double dval, int flags,
                                            int module_number);
ZEND_API void zend_register_string_constant(const char *name, size_t name_len,
                                            char *strval, int flags,
                                            int module_number);
ZEND_API void zend_register_stringl_constant(const char *name, size_t name_len,
                                             char *strval, size_t strlen,
                                             int flags, int module_number);
