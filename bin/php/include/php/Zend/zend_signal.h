...
typedef struct _zend_signal_globals_t {
  ...
  zend_signal_queue_t pstorage[ZEND_SIGNAL_QUEUE_SIZE], *phead, *ptail,
      *pavail; /* pending queue */
} zend_signal_globals_t;

#ifdef ZTS
#define SIGG(v)                                                                \
  ZEND_TSRMG_FAST(zend_signal_globals_offset, zend_signal_globals_t *, v)
BEGIN_EXTERN_C()
...
#define ZEND_SIGNAL_BLOCK_INTERRUPTIONS()                                      \
  if (EXPECTED(zend_signal_globals_id)) {                                      \
    SIGG(depth)                                                                \
    ++;                                                                        \
  }
...
ZEND_API int zend_sigaction(int signo, const struct sigaction *act,
