...
#define ZEND_STD_BUILD_OBJECT_PROPERTIES_ARRAY_COMPATIBLE(object) (																																						  /* We can use zend_std_build_object_properties_array() for objects    \
																																																						   * without properties ht and with standard handlers */                \
																   Z_OBJ_P(object)->properties == NULL && Z_OBJ_HT_P(object)->get_properties_for == NULL && Z_OBJ_HT_P(object)->get_properties == zend_std_get_properties /* For initialized proxies we need to forward to the real instance */ \
																   && (!zend_object_is_lazy_proxy(Z_OBJ_P(object)) || !zend_lazy_object_initialized(Z_OBJ_P(object))))
