...
#define zend_new_array(size)                                                                      \
	(__builtin_constant_p(size) ? ((((uint32_t)(size)) <= HT_MIN_SIZE) ? _zend_new_array_0()      \
...
static zend_always_inline zval *zend_hash_find_ind(const HashTable *ht, zend_string *key)
{
	...
	return (zv && Z_TYPE_P(zv) == IS_INDIRECT) ? ((Z_TYPE_P(Z_INDIRECT_P(zv)) != IS_UNDEF) ? Z_INDIRECT_P(zv) : NULL) : zv;
}
