...
ZEND_API void ZEND_FASTCALL zend_hash_extend(HashTable *ht, uint32_t nSize,
                                             zend_bool packed);
...
ZEND_API zval *ZEND_FASTCALL zend_hash_add_or_update(HashTable *ht,
                                                     ...
                                                     zval *pData,
                                                     uint32_t flag);
ZEND_API zval *ZEND_FASTCALL zend_hash_update(HashTable *ht, zend_string *key,
                                              zval *pData);
ZEND_API zval *ZEND_FASTCALL zend_hash_update_ind(HashTable *ht,
                                                  zend_string *key,
                                                  zval *pData);
ZEND_API zval *ZEND_FASTCALL zend_hash_add(HashTable *ht, zend_string *key,
                                           zval *pData);
ZEND_API zval *ZEND_FASTCALL zend_hash_add_new(HashTable *ht, zend_string *key,
                                               zval *pData);

ZEND_API zval *ZEND_FASTCALL zend_hash_str_add_or_update(
    HashTable *ht, const char *key, size_t len, zval *pData, uint32_t flag);
...
ZEND_API zval *ZEND_FASTCALL zend_hash_str_add(HashTable *ht, const char *key,
                                               size_t len, zval *pData);
...
ZEND_API zval *ZEND_FASTCALL zend_hash_index_add_or_update(HashTable *ht,
                                                           ...
                                                           zval *pData,
                                                           uint32_t flag);
ZEND_API zval *ZEND_FASTCALL zend_hash_index_add(HashTable *ht, zend_ulong h,
                                                 zval *pData);
ZEND_API zval *ZEND_FASTCALL zend_hash_index_add_new(HashTable *ht,
                                                     zend_ulong h, zval *pData);
ZEND_API zval *ZEND_FASTCALL zend_hash_index_update(HashTable *ht, zend_ulong h,
                                                    zval *pData);
...
ZEND_API zval *ZEND_FASTCALL zend_hash_set_bucket_key(HashTable *ht, Bucket *p,
                                                      zend_string *key);
...
ZEND_API void ZEND_FASTCALL zend_hash_apply_with_argument(
    HashTable *ht, apply_func_arg_t apply_func, void *);
...
ZEND_API int ZEND_FASTCALL zend_hash_str_del(HashTable *ht, const char *key,
                                             size_t len);
ZEND_API int ZEND_FASTCALL zend_hash_str_del_ind(HashTable *ht, const char *key,
                                                 size_t len);
...
/* The same as zend_hash_find(), but hash value of the key must be already
 * calculated */
ZEND_API zval *ZEND_FASTCALL _zend_hash_find_known_hash(const HashTable *ht,
                                                        zend_string *key);

static zend_always_inline zval *
zend_hash_find_ex(const HashTable *ht, zend_string *key, zend_bool known_hash) {
  if (known_hash) {
...
/* Misc */
static zend_always_inline zend_bool zend_hash_exists(const HashTable *ht,
                                                     zend_string *key) {
  return zend_hash_find(ht, key) != NULL;
}

static zend_always_inline zend_bool zend_hash_str_exists(const HashTable *ht,
                                                         const char *str,
                                                         size_t len) {
  return zend_hash_str_find(ht, str, len) != NULL;
}

static zend_always_inline zend_bool zend_hash_index_exists(const HashTable *ht,
                                                           zend_ulong h) {
  return zend_hash_index_find(ht, h) != NULL;
}

/* traversing */
ZEND_API HashPosition ZEND_FASTCALL
...
#define zend_hash_has_more_elements_ex(ht, pos)                                \
  (zend_hash_get_current_key_type_ex(ht, pos) == HASH_KEY_NON_EXISTENT         \
       ? FAILURE                                                               \
       : SUCCESS)
ZEND_API int ZEND_FASTCALL zend_hash_move_forward_ex(HashTable *ht,
                                                     HashPosition *pos);
ZEND_API int ZEND_FASTCALL zend_hash_move_backwards_ex(HashTable *ht,
                                                       HashPosition *pos);
ZEND_API int ZEND_FASTCALL zend_hash_get_current_key_ex(const HashTable *ht,
                                                        zend_string **str_index,
                                                        zend_ulong *num_index,
                                                        HashPosition *pos);
ZEND_API void ZEND_FASTCALL zend_hash_get_current_key_zval_ex(
    const HashTable *ht, zval *key, HashPosition *pos);
...
ZEND_API zval *ZEND_FASTCALL zend_hash_get_current_data_ex(HashTable *ht,
                                                           HashPosition *pos);
ZEND_API void ZEND_FASTCALL
zend_hash_internal_pointer_reset_ex(HashTable *ht, HashPosition *pos);
ZEND_API void ZEND_FASTCALL
zend_hash_internal_pointer_end_ex(HashTable *ht, HashPosition *pos);
...
#define zend_hash_get_current_key(ht, str_index, num_index)                    \
  zend_hash_get_current_key_ex(ht, str_index, num_index,                       \
                               &(ht)->nInternalPointer)
...
ZEND_API void ZEND_FASTCALL zend_hash_copy(HashTable *target, HashTable *source,
                                           copy_ctor_func_t pCopyConstructor);
ZEND_API void ZEND_FASTCALL zend_hash_merge(HashTable *target,
                                            HashTable *source,
                                            copy_ctor_func_t pCopyConstructor,
                                            zend_bool overwrite);
ZEND_API void ZEND_FASTCALL zend_hash_merge_ex(
    HashTable *target, HashTable *source, copy_ctor_func_t pCopyConstructor,
...
ZEND_API int zend_hash_compare(HashTable *ht1, HashTable *ht2,
                               compare_func_t compar, zend_bool ordered);
ZEND_API int ZEND_FASTCALL zend_hash_sort_ex(HashTable *ht,
...
#define zend_new_array(size)                                                   \
  (__builtin_constant_p(size)                                                  \
       ? ((((uint32_t)(size)) <= HT_MIN_SIZE) ? _zend_new_array_0()            \
...
ZEND_API HashTable *ZEND_FASTCALL
zend_proptable_to_symtable(HashTable *ht, zend_bool always_duplicate);

ZEND_API int ZEND_FASTCALL _zend_handle_numeric_str_ex(const char *key,
                                                       size_t length,
                                                       zend_ulong *idx);
...
ZEND_API HashPosition ZEND_FASTCALL
zend_hash_iterators_lower_pos(HashTable *ht, HashPosition start);
...
zend_hash_iterators_update(HashTable *ht, HashPosition from, HashPosition to) {
...
_zend_handle_numeric_str(const char *key, size_t length, zend_ulong *idx) {
  const char *tmp = key;
...
static zend_always_inline zval *zend_hash_find_ind(const HashTable *ht,
                                                   zend_string *key) {
  zval *zv;

  zv = zend_hash_find(ht, key);
  return (zv && Z_TYPE_P(zv) == IS_INDIRECT)
             ? ((Z_TYPE_P(Z_INDIRECT_P(zv)) != IS_UNDEF) ? Z_INDIRECT_P(zv)
                                                         : NULL)
             : zv;
}

static zend_always_inline zval *zend_hash_find_ex_ind(const HashTable *ht,
                                                      zend_string *key,
...
static zend_always_inline int zend_hash_exists_ind(const HashTable *ht,
                                                   zend_string *key) {
  zval *zv;
...
static zend_always_inline zval *
zend_hash_str_find_ind(const HashTable *ht, const char *str, size_t len) {
  zval *zv;
...
static zend_always_inline int
zend_hash_str_exists_ind(const HashTable *ht, const char *str, size_t len) {
  zval *zv;
...
static zend_always_inline zval *
zend_symtable_add_new(HashTable *ht, zend_string *key, zval *pData) {
  zend_ulong idx;
...
static zend_always_inline zval *
zend_symtable_update_ind(HashTable *ht, zend_string *key, zval *pData) {
  zend_ulong idx;
...
static zend_always_inline int zend_symtable_del(HashTable *ht,
                                                zend_string *key) {
  zend_ulong idx;
...
static zend_always_inline int zend_symtable_del_ind(HashTable *ht,
                                                    zend_string *key) {
  zend_ulong idx;
...
static zend_always_inline zval *zend_symtable_find(const HashTable *ht,
                                                   zend_string *key) {
  zend_ulong idx;
...
static zend_always_inline zval *zend_symtable_find_ind(const HashTable *ht,
                                                       zend_string *key) {
  zend_ulong idx;
...
static zend_always_inline int zend_symtable_exists(HashTable *ht,
                                                   zend_string *key) {
  zend_ulong idx;
...
static zend_always_inline int zend_symtable_exists_ind(HashTable *ht,
                                                       zend_string *key) {
  zend_ulong idx;
...
static zend_always_inline zval *zend_symtable_str_update(HashTable *ht,
                                                         const char *str,
                                                         size_t len,
                                                         zval *pData) {
  zend_ulong idx;
...
static zend_always_inline zval *zend_symtable_str_update_ind(HashTable *ht,
                                                             const char *str,
                                                             size_t len,
                                                             zval *pData) {
  zend_ulong idx;
...
static zend_always_inline int
zend_symtable_str_del(HashTable *ht, const char *str, size_t len) {
  zend_ulong idx;
...
static zend_always_inline zval *
zend_symtable_str_find(HashTable *ht, const char *str, size_t len) {
  zend_ulong idx;
...
static zend_always_inline int
zend_symtable_str_exists(HashTable *ht, const char *str, size_t len) {
  zend_ulong idx;
...
static zend_always_inline void *
zend_hash_add_ptr(HashTable *ht, zend_string *key, void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_add_new_ptr(HashTable *ht, zend_string *key, void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_str_add_ptr(HashTable *ht, const char *str, size_t len, void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *zend_hash_str_add_new_ptr(HashTable *ht,
                                                          const char *str,
                                                          size_t len,
                                                          void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_update_ptr(HashTable *ht, zend_string *key, void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_add_mem(HashTable *ht, zend_string *key, void *pData, size_t size) {
  zval tmp, *zv;
...
static zend_always_inline void *zend_hash_add_new_mem(HashTable *ht,
                                                      zend_string *key,
                                                      void *pData,
                                                      size_t size) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_str_add_new_mem(HashTable *ht, const char *str, size_t len,
...
static zend_always_inline void *
zend_hash_str_update_mem(HashTable *ht, const char *str, size_t len,
                         void *pData, size_t size) {
  void *p;
...
static zend_always_inline void *
zend_hash_index_add_ptr(HashTable *ht, zend_ulong h, void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_index_add_new_ptr(HashTable *ht, zend_ulong h, void *pData) {
  zval tmp, *zv;
...
static zend_always_inline void *
zend_hash_index_add_mem(HashTable *ht, zend_ulong h, void *pData, size_t size) {
  zval tmp, *zv;
...
static zend_always_inline void *zend_hash_index_update_mem(HashTable *ht,
                                                           zend_ulong h,
                                                           void *pData,
                                                           size_t size) {
  void *p;
...
static zend_always_inline void *
zend_hash_next_index_insert_mem(HashTable *ht, void *pData, size_t size) {
  zval tmp, *zv;
...
static zend_always_inline void *zend_hash_find_ex_ptr(const HashTable *ht,
                                                      zend_string *key,
                                                      zend_bool known_hash) {
  zval *zv;
...
static zend_always_inline zval *zend_hash_index_find_deref(HashTable *ht,
                                                           zend_ulong h) {
...
static zend_always_inline zval *zend_hash_find_deref(HashTable *ht,
                                                     zend_string *str) {
...
static zend_always_inline zval *
zend_hash_str_find_deref(HashTable *ht, const char *str, size_t len) {
...
static zend_always_inline void *
zend_symtable_str_find_ptr(HashTable *ht, const char *str, size_t len) {
  zend_ulong idx;
...
static zend_always_inline zval *
_zend_hash_append_ex(HashTable *ht, zend_string *key, zval *zv, int interned) {
...
static zend_always_inline zval *_zend_hash_append(HashTable *ht,
                                                  zend_string *key, zval *zv) {
  return _zend_hash_append_ex(ht, key, zv, 0);
}

static zend_always_inline zval *_zend_hash_append_ptr_ex(HashTable *ht,
                                                         zend_string *key,
                                                         void *ptr,
                                                         int interned) {
  uint32_t idx = ht->nNumUsed++;
...
static zend_always_inline zval *
_zend_hash_append_ptr(HashTable *ht, zend_string *key, void *ptr) {
  return _zend_hash_append_ptr_ex(ht, key, ptr, 0);
}

static zend_always_inline void
_zend_hash_append_ind(HashTable *ht, zend_string *key, zval *ptr) {
  uint32_t idx = ht->nNumUsed++;
  uint32_t nIndex;
  Bucket *p = ht->arData + idx;
