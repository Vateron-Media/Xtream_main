...
typedef enum {
  SUCCESS = 0,
  FAILURE =
      -1, /* this MUST stay a negative number, or it may affect functions! */
} ZEND_RESULT_CODE;
 ...
 * ZEND_TYPE_IS_CE()      - checks if type-hint refer to some class by
 * zend_class_entry * ZEND_TYPE_IS_NAME()    - checks if type-hint refer to some
 * class by zend_string *
 *
...
#define ZEND_TYPE_ENCODE_CLASS_CONST_Q1(allow_null, class_name)                \
  ZEND_TYPE_ENCODE_CLASS_CONST_Q2(ZEND_TYPE_ENCODE_CLASS_CONST_##allow_null,   \
                                  class_name)
...
struct _zend_array {
  ...
  union {
    struct {
      ZEND_ENDIAN_LOHI_4(zend_uchar flags, zend_uchar _unused,
                         zend_uchar nIteratorsCount, zend_uchar _unused2)
    } v;
    uint32_t flags;
  } u;
  uint32_t nTableMask;
...
#define HT_HASH_TO_BUCKET_EX(data, idx) ((Bucket *)((char *)(data) + (idx)))
#define HT_IDX_TO_HASH(idx) ((idx) * sizeof(Bucket))
#define HT_HASH_TO_IDX(idx) ((idx) / sizeof(Bucket))
...
#else
#define HT_HASH_RESET(ht)                                                      \
  memset(&HT_HASH(ht, (ht)->nTableMask), HT_INVALID_IDX,                       \
...
#define ZEND_PROPERTY_INFO_SOURCE_TO_LIST(list)                                \
  ((zend_property_info_list *)((list) & ~0x1))
...
#define ZEND_SAME_FAKE_TYPE(faketype, realtype)                                \
  ((faketype) == (realtype) ||                                                 \
   ((faketype) == _IS_BOOL &&                                                  \
    ((realtype) == IS_TRUE || (realtype) == IS_FALSE)))
...
#define Z_TYPE_INFO_REFCOUNTED(t)                                              \
  (((t) & (IS_TYPE_REFCOUNTED << Z_TYPE_FLAGS_SHIFT)) != 0)
#endif
...
#define IS_ARRAY_EX                                                            \
  (IS_ARRAY | (IS_TYPE_REFCOUNTED << Z_TYPE_FLAGS_SHIFT) |                     \
   (IS_TYPE_COLLECTABLE << Z_TYPE_FLAGS_SHIFT))
#define IS_OBJECT_EX                                                           \
  (IS_OBJECT | (IS_TYPE_REFCOUNTED << Z_TYPE_FLAGS_SHIFT) |                    \
   (IS_TYPE_COLLECTABLE << Z_TYPE_FLAGS_SHIFT))
#define IS_RESOURCE_EX                                                         \
  (IS_RESOURCE | (IS_TYPE_REFCOUNTED << Z_TYPE_FLAGS_SHIFT))
#define IS_REFERENCE_EX                                                        \
  (IS_REFERENCE | (IS_TYPE_REFCOUNTED << Z_TYPE_FLAGS_SHIFT))

#define IS_CONSTANT_AST_EX                                                     \
  (IS_CONSTANT_AST | (IS_TYPE_REFCOUNTED << Z_TYPE_FLAGS_SHIFT))
...
/* the following Z_OPT_* macros make better code when Z_TYPE_INFO accessed
 * before */
...
#define ZVAL_BOOL(z, b)                                                        \
  do {                                                                         \
    Z_TYPE_INFO_P(z) = (b) ? IS_TRUE : IS_FALSE;                               \
  } while (0)
...
#define ZVAL_STR(z, s)                                                         \
  do {                                                                         \
    ...
    Z_TYPE_INFO_P(__z) =                                                       \
        ZSTR_IS_INTERNED(__s) ? IS_INTERNED_STRING_EX : IS_STRING_EX;          \
  } while (0)
...
#define ZEND_RC_MOD_CHECK(p)                                                   \
  do {                                                                         \
    if (zend_rc_debug && zval_gc_type((p)->u.type_info) != IS_OBJECT) {        \
      ...
      ZEND_ASSERT((zval_gc_flags((p)->u.type_info) &                           \
                   (GC_PERSISTENT | GC_PERSISTENT_LOCAL)) != GC_PERSISTENT);   \
    }                                                                          \
  } while (0)
...
zend_gc_refcount(const zend_refcounted_h *p) {
  return p->refcount;
}

static zend_always_inline uint32_t zend_gc_set_refcount(zend_refcounted_h *p,
                                                        uint32_t rc) {
  p->refcount = rc;
  return p->refcount;
}
...
static zend_always_inline uint32_t zend_gc_addref_ex(zend_refcounted_h *p,
                                                     uint32_t rc) {
  ZEND_RC_MOD_CHECK(p);
  p->refcount += rc;
  return p->refcount;
...
static zend_always_inline uint32_t zend_gc_delref_ex(zend_refcounted_h *p,
                                                     uint32_t rc) {
  ZEND_RC_MOD_CHECK(p);
  p->refcount -= rc;
  return p->refcount;
...
#define SEPARATE_STRING(zv)                                                    \
  do {                                                                         \
    ...
    if (Z_REFCOUNT_P(_zv) > 1) {                                               \
      ...
      Z_DELREF_P(_zv);                                                         \
      ZVAL_NEW_STR(_zv, zend_string_init(ZSTR_VAL(_str), ZSTR_LEN(_str), 0));  \
    }                                                                          \
  } while (0)
 ...
 * the Z_EXTRA space when copying property default values etc. We define
 * separate macros for this purpose, so this workaround is easier to remove in
 * the future. */
