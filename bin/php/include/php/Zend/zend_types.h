...
#define ZEND_TYPE_INIT_CODE(code, allow_null, extra_flags) \
	ZEND_TYPE_INIT_MASK(((code) == _IS_BOOL ? MAY_BE_BOOL : ((code) == IS_ITERABLE ? _ZEND_TYPE_ITERABLE_BIT : ((code) == IS_MIXED ? MAY_BE_ANY : (1 << (code))))) | ((allow_null) ? _ZEND_TYPE_NULLABLE_BIT : 0) | (extra_flags))
...
#define ZEND_SAME_FAKE_TYPE(faketype, realtype) ( \
	(faketype) == (realtype) || ((faketype) == _IS_BOOL && ((realtype) == IS_TRUE || (realtype) == IS_FALSE)))
...
#define ZVAL_COPY_OR_DUP(z, v)                                                           \
	...
	{                                                                                    \
		...
		{                                                                                \
			...
			if (EXPECTED(!(GC_FLAGS(_gc) & GC_PERSISTENT) || GC_TYPE(_gc) == IS_OBJECT)) \
			{                                                                            \
				GC_ADDREF(_gc);                                                          \
			}                                                                            \
			else                                                                         \
