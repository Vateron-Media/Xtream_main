...
/* Compilation context that is different for each file, but shared between op
 * arrays. */
...
zend_op *zend_compile_var(znode *node, zend_ast *ast, uint32_t type,
                          int by_ref);
...
typedef struct _zend_property_info {
  uint32_t
      offset; /* property offset for object properties or
...
#define OP_JMP_ADDR(opline, node) (node).jmp_addr
...
#define ZEND_PASS_TWO_UPDATE_JMP_TARGET(op_array, opline, node)                \
  do {                                                                         \
    (node).jmp_offset =                                                        \
        ZEND_OPLINE_NUM_TO_OFFSET(op_array, opline, (node).opline_num);        \
  } while (0)
...
#define ZEND_PASS_TWO_UNDO_JMP_TARGET(op_array, opline, node)                  \
  do {                                                                         \
    (node).opline_num =                                                        \
        ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline, (node).jmp_offset);        \
  } while (0)
...
#define ZEND_PASS_TWO_UPDATE_CONSTANT(op_array, opline, node)                  \
  do {                                                                         \
    (node).constant = (((char *)CT_CONSTANT_EX(op_array, (node).constant)) -   \
                       ((char *)opline));                                      \
  } while (0)
...
extern ZEND_API zend_op_array *(*zend_compile_file)(
...
ZEND_API uint32_t
zend_build_delayed_early_binding_list(const zend_op_array *op_array);
ZEND_API void
zend_do_delayed_early_binding(zend_op_array *op_array,
                              uint32_t first_early_binding_opline);
...
ZEND_API void init_op_array(zend_op_array *op_array, zend_uchar type,
...
zend_mangle_property_name(const char *src1, size_t src1_length,
                          const char *src2, size_t src2_length, int internal);
...
ZEND_API int zend_unmangle_property_name_ex(const zend_string *name,
...
typedef zend_bool (*zend_needs_live_range_cb)(zend_op_array *op_array,
                                              zend_op *opline);
ZEND_API void
...
ZEND_API zend_uchar zend_get_call_op(const zend_op *init_op,
                                     zend_function *fbc);
...
ZEND_API int
zend_register_auto_global(zend_string *name, zend_bool jit,
                          zend_auto_global_callback auto_global_callback);
...
static zend_always_inline int zend_check_arg_send_type(const zend_function *zf,
                                                       uint32_t arg_num,
                                                       uint32_t mask) {
  arg_num--;
  ...
  return UNEXPECTED((zf->common.arg_info[arg_num].pass_by_reference & mask) !=
                    0);
}
...
#define ZEND_COMPILE_EXTENDED_INFO                                             \
  (ZEND_COMPILE_EXTENDED_STMT | ZEND_COMPILE_EXTENDED_FCALL)

/* call op_array handler of extendions */
#define ZEND_COMPILE_HANDLE_OP_ARRAY (1 << 2)

/* generate ZEND_INIT_FCALL_BY_NAME for internal functions instead of
 * ZEND_INIT_FCALL */
...
/* generate ZEND_INIT_FCALL_BY_NAME for userland functions instead of
 * ZEND_INIT_FCALL */
...
ZEND_API zend_bool zend_binary_op_produces_numeric_string_error(uint32_t opcode,
                                                                zval *op1,
                                                                zval *op2);
