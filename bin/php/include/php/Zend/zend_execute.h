...
#define CACHED_POLYMORPHIC_PTR(num, ce) \
	(EXPECTED(((void **)((char *)EX(run_time_cache) + (num)))[0] == (void *)(ce)) ? ((void **)((char *)EX(run_time_cache) + (num)))[1] : NULL)
...
#define SKIP_EXT_OPLINE(opline)                                                                 \
	do                                                                                          \
	{                                                                                           \
		while (UNEXPECTED((opline)->opcode >= ZEND_EXT_STMT && (opline)->opcode <= ZEND_TICKS)) \
		{                                                                                       \
			(opline)--;                                                                         \
		}                                                                                       \
	} while (0)
