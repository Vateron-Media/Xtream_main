...
ZEND_API extern void (*zend_execute_internal)(zend_execute_data *execute_data,
                                              zval *return_value);
...
ZEND_API int zend_eval_stringl_ex(char *str, size_t str_len, zval *retval_ptr,
...
zend_missing_arg_error(zend_execute_data *execute_data);

ZEND_API zend_bool ZEND_FASTCALL zend_verify_ref_assignable_zval(
    zend_reference *ref, zval *zv, zend_bool strict);
ZEND_API zend_bool ZEND_FASTCALL zend_verify_prop_assignable_by_ref(
    zend_property_info *prop_info, zval *orig_val, zend_bool strict);

ZEND_API ZEND_COLD void zend_throw_ref_type_error_zval(zend_property_info *prop,
                                                       zval *zv);
ZEND_API ZEND_COLD void
zend_throw_ref_type_error_type(zend_property_info *prop1,
                               zend_property_info *prop2, zval *zv);
...
ZEND_API void ZEND_FASTCALL zend_ref_add_type_source(
    zend_property_info_source_list *source_list, zend_property_info *prop);
ZEND_API void ZEND_FASTCALL zend_ref_del_type_source(
    zend_property_info_source_list *source_list, zend_property_info *prop);

ZEND_API zval *zend_assign_to_typed_ref(zval *variable_ptr, zval *value,
                                        zend_uchar value_type, zend_bool strict,
                                        zend_refcounted *ref);

static zend_always_inline void zend_copy_to_variable(zval *variable_ptr,
                                                     zval *value,
...
static zend_always_inline zval *zend_assign_to_variable(zval *variable_ptr,
                                                        zval *value,
                                                        zend_uchar value_type,
                                                        zend_bool strict) {
...
ZEND_API ZEND_COLD int zend_use_undefined_constant(zend_string *name,
                                                   zend_ast_attr attr,
                                                   zval *result);
...
#define ZEND_VM_STACK_HEADER_SLOTS                                             \
  ((ZEND_MM_ALIGNED_SIZE(sizeof(struct _zend_vm_stack)) +                      \
    ZEND_MM_ALIGNED_SIZE(sizeof(zval)) - 1) /                                  \
...
#define ZEND_ASSERT_VM_STACK(stack)                                            \
  ZEND_ASSERT(stack->top > (zval *)stack && stack->end > (zval *)stack &&      \
              stack->top <= stack->end)
#define ZEND_ASSERT_VM_STACK_GLOBAL                                            \
  ZEND_ASSERT(EG(vm_stack_top) > (zval *)EG(vm_stack) &&                       \
              EG(vm_stack_end) > (zval *)EG(vm_stack) &&                       \
              EG(vm_stack_top) <= EG(vm_stack_end))
#else
#define ZEND_ASSERT_VM_STACK(stack)
#define ZEND_ASSERT_VM_STACK_GLOBAL
#endif
...
zend_vm_init_call_frame(zend_execute_data *call, uint32_t call_info,
...
zend_vm_stack_push_call_frame_ex(uint32_t used_stack, uint32_t call_info,
  ...
  if (UNEXPECTED(used_stack >
                 (size_t)(((char *)EG(vm_stack_end)) - (char *)call))) {
    call = (zend_execute_data *)zend_vm_stack_extend(used_stack);
    ZEND_ASSERT_VM_STACK_GLOBAL;
    zend_vm_init_call_frame(call, call_info | ZEND_CALL_ALLOCATED, func,
...
zend_vm_calc_used_stack(uint32_t num_args, zend_function *func) {
...
static zend_always_inline void
zend_vm_stack_free_extra_args_ex(uint32_t call_info, zend_execute_data *call) {
  if (UNEXPECTED(call_info & ZEND_CALL_FREE_EXTRA_ARGS)) {
    uint32_t count = ZEND_CALL_NUM_ARGS(call) - call->func->op_array.num_args;
    zval *p = ZEND_CALL_VAR_NUM(call, call->func->op_array.last_var +
                                          call->func->op_array.T);
    do {
      if (Z_REFCOUNTED_P(p)) {
...
static zend_always_inline void
zend_vm_stack_free_extra_args(zend_execute_data *call) {
  zend_vm_stack_free_extra_args_ex(ZEND_CALL_INFO(call), call);
}

static zend_always_inline void
zend_vm_stack_free_args(zend_execute_data *call) {
...
static zend_always_inline void
zend_vm_stack_free_call_frame_ex(uint32_t call_info, zend_execute_data *call) {
  ZEND_ASSERT_VM_STACK_GLOBAL;
  ...
  if (UNEXPECTED(call_info & ZEND_CALL_ALLOCATED)) {
    ...
    ZEND_ASSERT(call ==
                (zend_execute_data *)ZEND_VM_STACK_ELEMENTS(EG(vm_stack)));
...
static zend_always_inline void
zend_vm_stack_free_call_frame(zend_execute_data *call) {
...
ZEND_API void zend_fetch_dimension_const(zval *result, zval *container,
                                         zval *dim, int type);
...
ZEND_API zval *zend_get_zval_ptr(const zend_op *opline, int op_type,
                                 const znode_op *node,
...
#define CACHED_POLYMORPHIC_PTR(num, ce)                                        \
  (EXPECTED(((void **)((char *)EX(run_time_cache) + (num)))[0] ==              \
            (void *)(ce))                                                      \
...
#define ZEND_CLASS_HAS_TYPE_HINTS(ce)                                          \
  ((ce->ce_flags & ZEND_ACC_HAS_TYPE_HINTS) == ZEND_ACC_HAS_TYPE_HINTS)

zend_bool zend_verify_property_type(zend_property_info *info, zval *property,
                                    zend_bool strict);
ZEND_COLD void zend_verify_property_type_error(zend_property_info *info,
                                               zval *property);
...
#define ZEND_REF_FOREACH_TYPE_SOURCES(ref, prop)                               \
  do {                                                                         \
    zend_property_info_source_list *_source_list =                             \
        &ZEND_REF_TYPE_SOURCES(ref);                                           \
