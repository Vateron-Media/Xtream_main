...
#define CHECK_ZVAL_STRING(str)                                                      \
	...
	{                                                                               \
		zend_error(E_WARNING, "String is not zero-terminated (%s)", ZSTR_VAL(str)); \
	}
#define CHECK_ZVAL_STRING_REL(str)                                                                                          \
	if (ZSTR_VAL(str)[ZSTR_LEN(str)] != '\0')                                                                               \
	{                                                                                                                       \
		zend_error(E_WARNING, "String is not zero-terminated (%s) (source: %s:%d)", ZSTR_VAL(str) ZEND_FILE_LINE_RELAY_CC); \
	}
#else
...
static zend_always_inline int zend_parse_arg_array_ht(zval *arg, HashTable **dest, int check_null, int or_object, int separate)
{
	...
	{
		if (separate && Z_OBJ_P(arg)->properties && UNEXPECTED(GC_REFCOUNT(Z_OBJ_P(arg)->properties) > 1))
		{
