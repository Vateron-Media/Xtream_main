...
#define ZEND_DEP_ME(classname, name, arg_info, flags)                          \
  ZEND_ME(classname, name, arg_info, flags | ZEND_ACC_DEPRECATED)
...
#define ZEND_ME_MAPPING(name, func_name, arg_types, flags)                     \
...
#define ZEND_ARG_ARRAY_INFO(pass_by_ref, name, allow_null)                     \
  {#name, ZEND_TYPE_ENCODE(IS_ARRAY, allow_null), pass_by_ref, 0},
...
#define ZEND_ARG_TYPE_INFO(pass_by_ref, name, type_hint, allow_null)           \
  {#name, ZEND_TYPE_ENCODE(type_hint, allow_null), pass_by_ref, 0},
...
#define ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(                                \
  ...
  static const zend_internal_arg_info name[] = {                               \
      {(const char *)(zend_uintptr_t)(required_num_args),                      \
       ZEND_TYPE_ENCODE_CLASS_CONST(#class_name, allow_null),                  \
       return_reference, 0},
...
#define ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(                               \
  ...
  static const zend_internal_arg_info name[] = {                               \
      {(const char *)(zend_uintptr_t)(required_num_args),                      \
       ZEND_TYPE_ENCODE(type, allow_null), return_reference, 0},
...
#define ZEND_BEGIN_ARG_INFO_EX(name, _unused, return_reference,                \
                               required_num_args)                              \
  static const zend_internal_arg_info name[] = {                               \
      {(const char *)(zend_uintptr_t)(required_num_args), 0, return_reference, \
       0},
...
#define ZEND_MODULE_STARTUP_D(module)                                          \
  int ZEND_MODULE_STARTUP_N(module)(INIT_FUNC_ARGS)
#define ZEND_MODULE_SHUTDOWN_D(module)                                         \
  int ZEND_MODULE_SHUTDOWN_N(module)(SHUTDOWN_FUNC_ARGS)
#define ZEND_MODULE_ACTIVATE_D(module)                                         \
  int ZEND_MODULE_ACTIVATE_N(module)(INIT_FUNC_ARGS)
...
#define ZEND_MODULE_POST_ZEND_DEACTIVATE_D(module)                             \
  int ZEND_MODULE_POST_ZEND_DEACTIVATE_N(module)(void)
#define ZEND_MODULE_INFO_D(module)                                             \
  ZEND_COLD void ZEND_MODULE_INFO_N(module)(ZEND_MODULE_INFO_FUNC_ARGS)
#define ZEND_MODULE_GLOBALS_CTOR_D(module)                                     \
  void ZEND_MODULE_GLOBALS_CTOR_N(module)(zend_##module##_globals *            \
                                          module##_globals)
...
#define ZEND_INIT_MODULE_GLOBALS(module_name, globals_ctor, globals_dtor)      \
  ts_allocate_id(                                                              \
      &module_name##_globals_id, sizeof(zend_##module_name##_globals),         \
      (ts_allocate_ctor)globals_ctor, (ts_allocate_dtor)globals_dtor);
#define ZEND_MODULE_GLOBALS_ACCESSOR(module_name, v)                           \
  ZEND_TSRMG(module_name##_globals_id, zend_##module_name##_globals *, v)
#if ZEND_ENABLE_STATIC_TSRMLS_CACHE
#define ZEND_MODULE_GLOBALS_BULK(module_name)                                  \
  TSRMG_BULK_STATIC(module_name##_globals_id, zend_##module_name##_globals *)
#else
#define ZEND_MODULE_GLOBALS_BULK(module_name)                                  \
...
#define INIT_CLASS_ENTRY(class_container, class_name, functions)               \
  INIT_CLASS_ENTRY_EX(class_container, class_name, sizeof(class_name) - 1,     \
                      functions)

#define INIT_CLASS_ENTRY_EX(class_container, class_name, class_name_len,       \
                            functions)                                         \
  {                                                                            \
...
#define zend_parse_parameters_none()                                           \
  (EXPECTED(ZEND_NUM_ARGS() == 0)                                              \
       ? SUCCESS                                                               \
       : (zend_wrong_parameters_none_error(), FAILURE))
#define zend_parse_parameters_none_throw()                                     \
  (EXPECTED(ZEND_NUM_ARGS() == 0)                                              \
       ? SUCCESS                                                               \
       : (zend_wrong_parameters_none_exception(), FAILURE))
...
ZEND_API int zend_parse_parameters_ex(int flags, int num_args,
...
ZEND_API int zend_parse_method_parameters(int num_args, zval *this_ptr,
                                          const char *type_spec, ...);
ZEND_API int zend_parse_method_parameters_ex(int flags, int num_args,
                                             zval *this_ptr,
                                             const char *type_spec, ...);

ZEND_API int zend_parse_parameter(int flags, int arg_num, zval *arg,
                                  const char *spec, ...);
...
ZEND_API int zend_register_class_alias_ex(const char *name, size_t name_len,
                                          zend_class_entry *ce, int persistent);

#define zend_register_class_alias(name, ce)                                    \
  zend_register_class_alias_ex(name, sizeof(name) - 1, ce, 1)
#define zend_register_ns_class_alias(ns, name, ce)                             \
  zend_register_class_alias_ex(ZEND_NS_NAME(ns, name),                         \
                               sizeof(ZEND_NS_NAME(ns, name)) - 1, ce, 1)
...
ZEND_API zend_bool zend_is_callable_ex(zval *callable, zend_object *object,
                                       uint32_t check_flags,
                                       zend_string **callable_name,
                                       zend_fcall_info_cache *fcc,
                                       char **error);
ZEND_API zend_bool zend_is_callable(zval *callable, uint32_t check_flags,
                                    zend_string **callable_name);
...
ZEND_API int zend_declare_typed_property(zend_class_entry *ce,
                                         zend_string *name, zval *property,
                                         int access_type,
                                         zend_string *doc_comment,
                                         zend_type type);

ZEND_API int zend_declare_property_ex(zend_class_entry *ce, zend_string *name,
                                      zval *property, int access_type,
                                      zend_string *doc_comment);
ZEND_API int zend_declare_property(zend_class_entry *ce, const char *name,
                                   size_t name_length, zval *property,
                                   int access_type);
...
ZEND_API void zend_update_property_ex(zend_class_entry *scope, zval *object,
                                      zend_string *name, zval *value);
...
ZEND_API void zend_update_property_double(zend_class_entry *scope, zval *object,
                                          const char *name, size_t name_length,
                                          double value);
ZEND_API void zend_update_property_str(zend_class_entry *scope, zval *object,
                                       const char *name, size_t name_length,
                                       zend_string *value);
...
ZEND_API void zend_update_property_stringl(zend_class_entry *scope,
                                           ...
                                           const char *value,
                                           size_t value_length);
ZEND_API void zend_unset_property(zend_class_entry *scope, zval *object,
                                  const char *name, size_t name_length);

ZEND_API int zend_update_static_property_ex(zend_class_entry *scope,
                                            zend_string *name, zval *value);
...
ZEND_API zval *zend_read_property_ex(zend_class_entry *scope, zval *object,
                                     zend_string *name, zend_bool silent,
                                     zval *rv);
ZEND_API zval *zend_read_property(zend_class_entry *scope, zval *object,
                                  const char *name, size_t name_length,
                                  zend_bool silent, zval *rv);
...
#define WRONG_PARAM_COUNT_WITH_RETVAL(ret)                                     \
  ZEND_WRONG_PARAM_COUNT_WITH_RETVAL(ret)
...
#define ZEND_WRONG_PARAM_COUNT_WITH_RETVAL(ret)                                \
  {                                                                            \
    zend_wrong_param_count();                                                  \
    return ret;                                                                \
  }
...
ZEND_API int object_and_properties_init(zval *arg, zend_class_entry *ce,
...
ZEND_API int add_assoc_long_ex(zval *arg, const char *key, size_t key_len,
                               zend_long n);
ZEND_API int add_assoc_null_ex(zval *arg, const char *key, size_t key_len);
ZEND_API int add_assoc_bool_ex(zval *arg, const char *key, size_t key_len,
                               int b);
ZEND_API int add_assoc_resource_ex(zval *arg, const char *key, size_t key_len,
                                   zend_resource *r);
ZEND_API int add_assoc_double_ex(zval *arg, const char *key, size_t key_len,
                                 double d);
ZEND_API int add_assoc_str_ex(zval *arg, const char *key, size_t key_len,
                              zend_string *str);
ZEND_API int add_assoc_string_ex(zval *arg, const char *key, size_t key_len,
                                 const char *str);
ZEND_API int add_assoc_stringl_ex(zval *arg, const char *key, size_t key_len,
                                  const char *str, size_t length);
ZEND_API int add_assoc_zval_ex(zval *arg, const char *key, size_t key_len,
                               zval *value);

#define add_assoc_long(__arg, __key, __n)                                      \
  add_assoc_long_ex(__arg, __key, strlen(__key), __n)
#define add_assoc_null(__arg, __key)                                           \
  add_assoc_null_ex(__arg, __key, strlen(__key))
#define add_assoc_bool(__arg, __key, __b)                                      \
  add_assoc_bool_ex(__arg, __key, strlen(__key), __b)
#define add_assoc_resource(__arg, __key, __r)                                  \
  add_assoc_resource_ex(__arg, __key, strlen(__key), __r)
#define add_assoc_double(__arg, __key, __d)                                    \
  add_assoc_double_ex(__arg, __key, strlen(__key), __d)
#define add_assoc_str(__arg, __key, __str)                                     \
...
#define add_assoc_stringl(__arg, __key, __str, __length)                       \
  add_assoc_stringl_ex(__arg, __key, strlen(__key), __str, __length)
#define add_assoc_zval(__arg, __key, __value)                                  \
  add_assoc_zval_ex(__arg, __key, strlen(__key), __value)
...
ZEND_API int add_index_stringl(zval *arg, zend_ulong index, const char *str,
                               size_t length);

static zend_always_inline int add_index_zval(zval *arg, zend_ulong index,
                                             zval *value) {
  return zend_hash_index_update(Z_ARRVAL_P(arg), index, value) ? SUCCESS
                                                               : FAILURE;
}
...
static zend_always_inline int add_next_index_zval(zval *arg, zval *value) {
  return zend_hash_next_index_insert(Z_ARRVAL_P(arg), value) ? SUCCESS
                                                             : FAILURE;
}

ZEND_API int array_set_zval_key(HashTable *ht, zval *key, zval *value);

ZEND_API int add_property_long_ex(zval *arg, const char *key, size_t key_len,
                                  zend_long l);
ZEND_API int add_property_null_ex(zval *arg, const char *key, size_t key_len);
ZEND_API int add_property_bool_ex(zval *arg, const char *key, size_t key_len,
                                  zend_long b);
ZEND_API int add_property_resource_ex(zval *arg, const char *key,
                                      size_t key_len, zend_resource *r);
ZEND_API int add_property_double_ex(zval *arg, const char *key, size_t key_len,
                                    double d);
ZEND_API int add_property_str_ex(zval *arg, const char *key, size_t key_len,
                                 zend_string *str);
ZEND_API int add_property_string_ex(zval *arg, const char *key, size_t key_len,
                                    const char *str);
ZEND_API int add_property_stringl_ex(zval *arg, const char *key, size_t key_len,
                                     const char *str, size_t length);
ZEND_API int add_property_zval_ex(zval *arg, const char *key, size_t key_len,
                                  zval *value);

#define add_property_long(__arg, __key, __n)                                   \
  add_property_long_ex(__arg, __key, strlen(__key), __n)
#define add_property_null(__arg, __key)                                        \
  add_property_null_ex(__arg, __key, strlen(__key))
#define add_property_bool(__arg, __key, __b)                                   \
  add_property_bool_ex(__arg, __key, strlen(__key), __b)
#define add_property_resource(__arg, __key, __r)                               \
  add_property_resource_ex(__arg, __key, strlen(__key), __r)
#define add_property_double(__arg, __key, __d)                                 \
  add_property_double_ex(__arg, __key, strlen(__key), __d)
#define add_property_str(__arg, __key, __str)                                  \
...
#define add_property_stringl(__arg, __key, __str, __length)                    \
  add_property_stringl_ex(__arg, __key, strlen(__key), __str, __length)
#define add_property_zval(__arg, __key, __value)                               \
  add_property_zval_ex(__arg, __key, strlen(__key), __value)

ZEND_API int _call_user_function_ex(zval *object, zval *function_name,
                                    zval *retval_ptr, uint32_t param_count,
                                    zval params[], int no_separation);

#define call_user_function(function_table, object, function_name, retval_ptr,  \
                           param_count, params)                                \
  _call_user_function_ex(object, function_name, retval_ptr, param_count,       \
                         params, 1)
#define call_user_function_ex(function_table, object, function_name,           \
                              retval_ptr, param_count, params, no_separation,  \
                              symbol_table)                                    \
  _call_user_function_ex(object, function_name, retval_ptr, param_count,       \
                         params, no_separation)
 ...
 *
 * Caller is responsible to provide a return value (fci->retval), otherwise the
 * we will crash. In order to pass parameters the following members need to be
 * set: fci->param_count = 0; fci->params = NULL; The callable_name argument may
 * be NULL. Set check_flags to IS_CALLABLE_STRICT for every new usage!
 */
ZEND_API int zend_fcall_info_init(zval *callable, uint32_t check_flags,
                                  zend_fcall_info *fci,
                                  zend_fcall_info_cache *fcc,
...
ZEND_API int zend_fcall_info_args_ex(zend_fcall_info *fci, zend_function *func,
                                     zval *args);
 ...
 * a variable amount of zval** arguments.
 */
ZEND_API int zend_fcall_info_argv(zend_fcall_info *fci, int argc,
                                  va_list *argv);
...
ZEND_API int zend_set_local_var_str(const char *name, size_t len, zval *value,
                                    int force);
...
ZEND_API zend_string *zend_find_alias_name(zend_class_entry *ce,
                                           zend_string *name);
ZEND_API zend_string *zend_resolve_method_name(zend_class_entry *ce,
                                               zend_function *f);
...
#define CHECK_ZVAL_STRING(str)                                                 \
  if (ZSTR_VAL(str)[ZSTR_LEN(str)] != '\0') {                                  \
    zend_error(E_WARNING, "String is not zero-terminated (%s)",                \
               ZSTR_VAL(str));                                                 \
  }
#define CHECK_ZVAL_STRING_REL(str)                                             \
  if (ZSTR_VAL(str)[ZSTR_LEN(str)] != '\0') {                                  \
    zend_error(E_WARNING,                                                      \
               ...
               ZSTR_VAL(str) ZEND_FILE_LINE_RELAY_CC);                         \
  }
#else
...
#define HASH_OF(p)                                                             \
  (Z_TYPE_P(p) == IS_ARRAY                                                     \
       ? Z_ARRVAL_P(p)                                                         \
       : ((Z_TYPE_P(p) == IS_OBJECT ? Z_OBJ_HT_P(p)->get_properties((p))       \
                                    : NULL)))
...
/* May modify arg in-place. Will free arg in failure case (and take ownership in
 * success case). Prefer using the ZEND_TRY_ASSIGN_* macros over these APIs. */
ZEND_API int zend_try_assign_typed_ref_ex(zend_reference *ref, zval *zv,
                                          zend_bool strict);
...
ZEND_API int zend_try_assign_typed_ref_long(zend_reference *ref,
                                            zend_long lval);
...
ZEND_API int zend_try_assign_typed_ref_str(zend_reference *ref,
                                           zend_string *str);
ZEND_API int zend_try_assign_typed_ref_string(zend_reference *ref,
                                              const char *string);
ZEND_API int zend_try_assign_typed_ref_stringl(zend_reference *ref,
                                               const char *string, size_t len);
ZEND_API int zend_try_assign_typed_ref_arr(zend_reference *ref,
                                           zend_array *arr);
ZEND_API int zend_try_assign_typed_ref_res(zend_reference *ref,
                                           zend_resource *res);
ZEND_API int zend_try_assign_typed_ref_zval(zend_reference *ref, zval *zv);
ZEND_API int zend_try_assign_typed_ref_zval_ex(zend_reference *ref, zval *zv,
                                               zend_bool strict);
...
static zend_always_inline zval *zend_try_array_init_size(zval *zv,
                                                         uint32_t size) {
...
typedef enum _zend_expected_type {
  Z_EXPECTED_TYPES(Z_EXPECTED_TYPE_ENUM) Z_EXPECTED_LAST
} zend_expected_type;
...
zend_wrong_parameters_count_error(int min_num_args, int max_num_args);
ZEND_API ZEND_COLD void ZEND_FASTCALL
zend_wrong_parameters_count_exception(int min_num_args, int max_num_args);
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_wrong_parameter_type_error(
    int num, zend_expected_type expected_type, zval *arg);
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_wrong_parameter_type_exception(
    int num, zend_expected_type expected_type, zval *arg);
ZEND_API ZEND_COLD void ZEND_FASTCALL
zend_wrong_parameter_class_error(int num, char *name, zval *arg);
ZEND_API ZEND_COLD void ZEND_FASTCALL
zend_wrong_parameter_class_exception(int num, char *name, zval *arg);
ZEND_API ZEND_COLD void ZEND_FASTCALL zend_wrong_callback_error(int num,
                                                                char *error);
ZEND_API ZEND_COLD void ZEND_FASTCALL
zend_wrong_callback_deprecated(int num, char *error);
ZEND_API ZEND_COLD void ZEND_FASTCALL
zend_wrong_callback_exception(int num, char *error);
      ...
      if (UNEXPECTED(_num_args < _min_num_args) ||                             \
        ...
        if (!(_flags & ZEND_PARSE_PARAMS_QUIET)) {                             \
          if (_flags & ZEND_PARSE_PARAMS_THROW) {                              \
            zend_wrong_parameters_count_exception(_min_num_args,               \
                                                  _max_num_args);              \
          } else {                                                             \
...
#define Z_PARAM_FUNC_EX2(dest_fci, dest_fcc, check_null, deref, separate)      \
  Z_PARAM_PROLOGUE(deref, separate);                                           \
  if (UNEXPECTED(!zend_parse_arg_func(_arg, &dest_fci, &dest_fcc, check_null,  \
                                      &_error))) {                             \
    if (!_error) {                                                             \
...
#define Z_PARAM_ARRAY_HT_EX2(dest, check_null, deref, separate)                \
  Z_PARAM_PROLOGUE(deref, separate);                                           \
  if (UNEXPECTED(                                                              \
          !zend_parse_arg_array_ht(_arg, &dest, check_null, 0, separate))) {   \
    _expected_type = Z_EXPECTED_ARRAY;                                         \
    _error_code = ZPP_ERROR_WRONG_ARG;                                         \
    break;                                                                     \
  }
...
#define Z_PARAM_ARRAY_OR_OBJECT_HT_EX2(dest, check_null, deref, separate)      \
  Z_PARAM_PROLOGUE(deref, separate);                                           \
  if (UNEXPECTED(                                                              \
          !zend_parse_arg_array_ht(_arg, &dest, check_null, 1, separate))) {   \
    _expected_type = Z_EXPECTED_ARRAY;                                         \
    _error_code = ZPP_ERROR_WRONG_ARG;                                         \
    break;                                                                     \
  }
...
#define Z_PARAM_LONG_EX2(dest, is_null, check_null, deref, separate)           \
  Z_PARAM_PROLOGUE(deref, separate);                                           \
  if (UNEXPECTED(                                                              \
          !zend_parse_arg_long(_arg, &dest, &is_null, check_null, 0))) {       \
...
#define Z_PARAM_STRICT_LONG_EX2(dest, is_null, check_null, deref, separate)    \
  Z_PARAM_PROLOGUE(deref, separate);                                           \
  if (UNEXPECTED(                                                              \
          !zend_parse_arg_long(_arg, &dest, &is_null, check_null, 1))) {       \
    _expected_type = Z_EXPECTED_LONG;                                          \
    _error_code = ZPP_ERROR_WRONG_ARG;                                         \
    break;                                                                     \
  }
...
#define Z_PARAM_STRING_EX2(dest, dest_len, check_null, deref, separate)        \
  Z_PARAM_PROLOGUE(deref, separate);                                           \
  if (UNEXPECTED(                                                              \
          !zend_parse_arg_string(_arg, &dest, &dest_len, check_null))) {       \
...
ZEND_API int ZEND_FASTCALL zend_parse_arg_class(zval *arg,
                                                zend_class_entry **pce, int num,
                                                int check_null);
...
ZEND_API int ZEND_FASTCALL zend_parse_arg_long_cap_slow(zval *arg,
                                                        zend_long *dest);
ZEND_API int ZEND_FASTCALL zend_parse_arg_long_cap_weak(zval *arg,
                                                        zend_long *dest);
...
ZEND_API int ZEND_FASTCALL zend_parse_arg_str_slow(zval *arg,
                                                   zend_string **dest);
ZEND_API int ZEND_FASTCALL zend_parse_arg_str_weak(zval *arg,
                                                   zend_string **dest);

static zend_always_inline int zend_parse_arg_bool(zval *arg, zend_bool *dest,
                                                  zend_bool *is_null,
                                                  int check_null) {
  if (check_null) {
    *is_null = 0;
...
static zend_always_inline int zend_parse_arg_long(zval *arg, zend_long *dest,
                                                  zend_bool *is_null,
                                                  int check_null, int cap) {
  if (check_null) {
    *is_null = 0;
  }
...
static zend_always_inline int zend_parse_arg_str(zval *arg, zend_string **dest,
                                                 int check_null) {
...
static zend_always_inline int zend_parse_arg_string(zval *arg, char **dest,
                                                    size_t *dest_len,
                                                    int check_null) {
  zend_string *str;
...
zend_parse_arg_path_str(zval *arg, zend_string **dest, int check_null) {
  if (!zend_parse_arg_str(arg, dest, check_null) ||
      (*dest &&
       UNEXPECTED(CHECK_NULL_PATH(ZSTR_VAL(*dest), ZSTR_LEN(*dest))))) {
    return 0;
  }
  return 1;
}
...
static zend_always_inline int
zend_parse_arg_path(zval *arg, char **dest, size_t *dest_len, int check_null) {
  zend_string *str;
...
static zend_always_inline int
zend_parse_arg_array(zval *arg, zval **dest, int check_null, int or_object) {
...
static zend_always_inline int
zend_parse_arg_array_ht(zval *arg, HashTable **dest, int check_null,
                        int or_object, int separate) {
  if (EXPECTED(Z_TYPE_P(arg) == IS_ARRAY)) {
    *dest = Z_ARRVAL_P(arg);
  } else if (or_object && EXPECTED(Z_TYPE_P(arg) == IS_OBJECT)) {
    if (separate && Z_OBJ_P(arg)->properties &&
        UNEXPECTED(GC_REFCOUNT(Z_OBJ_P(arg)->properties) > 1)) {
      if (EXPECTED(
              !(GC_FLAGS(Z_OBJ_P(arg)->properties) & IS_ARRAY_IMMUTABLE))) {
...
static zend_always_inline int zend_parse_arg_resource(zval *arg, zval **dest,
                                                      int check_null) {
...
static zend_always_inline int
zend_parse_arg_func(zval *arg, zend_fcall_info *dest_fci,
                    zend_fcall_info_cache *dest_fcc, int check_null,
                    char **error) {
  if (check_null && UNEXPECTED(Z_TYPE_P(arg) == IS_NULL)) {
    dest_fci->size = 0;
    dest_fcc->function_handler = NULL;
    *error = NULL;
  } else if (UNEXPECTED(zend_fcall_info_init(arg, 0, dest_fci, dest_fcc, NULL,
                                             error) != SUCCESS)) {
    return 0;
  }
  return 1;
...
static zend_always_inline void zend_parse_arg_zval(zval *arg, zval **dest,
                                                   int check_null) {
  *dest = (check_null && (UNEXPECTED(Z_TYPE_P(arg) == IS_NULL) ||
                          (UNEXPECTED(Z_ISREF_P(arg)) &&
                           UNEXPECTED(Z_TYPE_P(Z_REFVAL_P(arg)) == IS_NULL))))
              ? NULL
              : arg;
}

static zend_always_inline void zend_parse_arg_zval_deref(zval *arg, zval **dest,
                                                         int check_null) {
