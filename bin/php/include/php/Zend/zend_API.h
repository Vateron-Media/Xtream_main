...
static zend_always_inline bool zend_fcc_equals(const zend_fcall_info_cache *a, const zend_fcall_info_cache *b)
{
	...
	{
		return a->object == b->object && a->calling_scope == b->calling_scope && a->closure == b->closure && zend_string_equals(a->function_handler->common.function_name, b->function_handler->common.function_name);
	}
	return a->function_handler == b->function_handler && a->object == b->object && a->calling_scope == b->calling_scope && a->closure == b->closure;
}
...
#define RETURN_STRING_FAST(s)  \
	do                         \
	{                          \
		RETVAL_STRING_FAST(s); \
		return;                \
	} while (0)
#define RETURN_STRINGL_FAST(s, l)  \
	do                             \
	{                              \
		RETVAL_STRINGL_FAST(s, l); \
		return;                    \
	} while (0)
...
#define RETURN_COPY_VALUE(zv)  \
	do                         \
	{                          \
		RETVAL_COPY_VALUE(zv); \
		return;                \
	} while (0)
#define RETURN_COPY_DEREF(zv)  \
	do                         \
	{                          \
		RETVAL_COPY_DEREF(zv); \
		return;                \
	} while (0)
