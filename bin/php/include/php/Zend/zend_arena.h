...
static zend_always_inline void *zend_arena_alloc(zend_arena **arena_ptr,
                                                 size_t size) {
  ...
  if (EXPECTED(size <= (size_t)(arena->end - ptr))) {
    ...
    size_t arena_size =
        UNEXPECTED((size + ZEND_MM_ALIGNED_SIZE(sizeof(zend_arena))) >
    ...
    new_arena->ptr =
        (char *)new_arena + ZEND_MM_ALIGNED_SIZE(sizeof(zend_arena)) + size;
...
zend_arena_calloc(zend_arena **arena_ptr, size_t count, size_t unit_size) {
  int overflow;
  size_t size;
  void *ret;
...
static zend_always_inline void zend_arena_release(zend_arena **arena_ptr,
                                                  void *checkpoint) {
  zend_arena *arena = *arena_ptr;
  ...
  ZEND_ASSERT((char *)checkpoint > (char *)arena &&
              (char *)checkpoint <= arena->end);
  arena->ptr = (char *)checkpoint;
}

static zend_always_inline zend_bool zend_arena_contains(zend_arena *arena,
                                                        void *ptr) {
  while (arena) {
