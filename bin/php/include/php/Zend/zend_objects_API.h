...
#define SET_OBJ_INVALID(o)                                                     \
  ((zend_object *)((((zend_uintptr_t)(o)) | OBJ_BUCKET_INVALID)))
...
#define ZEND_OBJECTS_STORE_ADD_TO_FREE_LIST(h)                                 \
  do {                                                                         \
    SET_OBJ_BUCKET_NUMBER(EG(objects_store).object_buckets[(h)],               \
...
ZEND_API void ZEND_FASTCALL zend_objects_store_free_object_storage(
    zend_objects_store *objects, zend_bool fast_shutdown);
ZEND_API void ZEND_FASTCALL
...
zend_object_properties_size(zend_class_entry *ce) {
...
static zend_always_inline void *zend_object_alloc(size_t obj_size,
                                                  zend_class_entry *ce) {
  void *obj = emalloc(obj_size + zend_object_properties_size(ce));
  /* Subtraction of sizeof(zval) is necessary, because
   * zend_object_properties_size() may be -sizeof(zval), if the object has no
   * properties. */
...
/* Helper for cases where we're only interested in property info of typed
 * properties. */
static inline zend_property_info *
