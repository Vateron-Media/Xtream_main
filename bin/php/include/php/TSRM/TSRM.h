...
#if SIZEOF_SIZE_T == 4
#define TSRM_ALIGNED_SIZE(size) (((size) + INT32_C(15)) & ~INT32_C(15))
#else
#define TSRM_ALIGNED_SIZE(size) (((size) + INT64_C(15)) & ~INT64_C(15))
#endif
...
/* startup/shutdown */
TSRM_API int tsrm_startup(int expected_threads, int expected_resources,
                          int debug_level, char *debug_filename);
...
TSRM_API ts_rsrc_id ts_allocate_fast_id(ts_rsrc_id *rsrc_id, size_t *offset,
...
TSRM_API void *tsrm_set_new_thread_begin_handler(
    tsrm_thread_begin_func_t new_thread_begin_handler);
TSRM_API void *
tsrm_set_new_thread_end_handler(tsrm_thread_end_func_t new_thread_end_handler);
 ...
 * threading model used by PHP/Zend and the selected SAPI. */
...
#define TSRMG_FAST(offset, type, element)                                      \
  (TSRMG_FAST_BULK(offset, type)->element)
#define TSRMG_FAST_BULK(offset, type)                                          \
  ((type)(((char *)tsrm_get_ls_cache()) + (offset)))

#define TSRMG_STATIC(id, type, element) (TSRMG_BULK_STATIC(id, type)->element)
#define TSRMG_BULK_STATIC(id, type)                                            \
  ((type)(*((void ***)TSRMLS_CACHE))[TSRM_UNSHUFFLE_RSRC_ID(id)])
#define TSRMG_FAST_STATIC(offset, type, element)                               \
  (TSRMG_FAST_BULK_STATIC(offset, type)->element)
#define TSRMG_FAST_BULK_STATIC(offset, type)                                   \
  ((type)(((char *)TSRMLS_CACHE) + (offset)))
