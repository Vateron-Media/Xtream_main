...
def prepare():
    ...
    os.system(
        "add-apt-repository ppa:linuxuprising/libpng12 -y > /dev/null && apt-get update > /dev/null"
    )
...
def install():
    ...
    if not "/sbin/iptables, /usr/bin/chattr" in open("/etc/sudoers").read():
        os.system(
            'sed -i "s|xtreamcodes|#xtreamcodes|g" /etc/sudoers && echo "xtreamcodes ALL=(root) NOPASSWD: /sbin/iptables, /usr/bin/chattr" >> /etc/sudoers'
        )
    ...
    if os.path.exists("/tmp/xtreamcodes.tar.gz"):
        ...
        os.system(
            'wget -q -O "/home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb" "%s"' % rGeo
        )
        os.system("chown -R xtreamcodes:xtreamcodes /home/xtreamcodes/")
        os.system("mount -a")
        os.system(
            'mv "%s.xc" "%s" && mv "%s.xc" "%s" && mv "%s.xc" "%s"'
            % (rNginx, rNginx, rNginxRtmp, rNginxRtmp, rIni, rIni)
        )
    ...
    if not os.path.exists("/etc/init.d/xtreamcodes"):
        os.system("touch /etc/init.d/xtreamcodes")
...
def start():
    ...
    os.system(
        "wget http://xtream-ui.mine.nu/GeoLite2.mmdb -O /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb -o /dev/null"
    )
    os.system("chown -R xtreamcodes:xtreamcodes /home/xtreamcodes")
    os.system(
