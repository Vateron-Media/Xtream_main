...
if (@$argc) {
    ...
    $stream_ids = array();
    $ipTV_db->query("SELECT t2.stream_display_name, t1.stream_id, t1.monitor_pid, t1.on_demand, t1.server_stream_id, t1.pid, clients.online_clients FROM `streams_sys` t1 INNER JOIN `streams` t2 ON t2.id = t1.stream_id AND t2.direct_source = 0 INNER JOIN `streams_types` t3 ON t3.type_id = t2.type LEFT JOIN ( SELECT stream_id, COUNT(*) as online_clients FROM `user_activity_now` WHERE `server_id` = '%d' GROUP BY stream_id) AS clients ON clients.stream_id = t1.stream_id WHERE (t1.pid IS NOT NULL OR t1.stream_status <> 0 OR t1.to_analyze = 1 ) AND t1.server_id = '%d' AND t3.live = 1", SERVER_ID, SERVER_ID);
            ...
            if (ipTV_streaming::CheckPidExist($stream["monitor_pid"], $stream["stream_id"])) {
                if (!($stream["on_demand"] == 1 && $stream["online_clients"] == 0)) {
                    ...
                    if (ipTV_streaming::CheckPidChannelM3U8Exist($stream["pid"], $stream["stream_id"]) && file_exists($playlist)) {
                        ...
                        $progressFile = file_exists(STREAMS_PATH . $stream["stream_id"] . "_.progress") ? json_decode(file_get_contents(STREAMS_PATH . $stream["stream_id"] . "_.progress"), true) : [];
