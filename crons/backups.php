...
if (posix_getpwuid(posix_geteuid())['name'] == 'xtreamcodes') {
    ...
    $bDoBackup = false;
    if (isset($rSettings['automatic_backups']) && !empty($rSettings['automatic_backups'])) {
        if ($rSettings['automatic_backups'] == 'hourly' && $rAdminSettings['automatic_backups_check'] < (time() - 3600)) {
            $bDoBackup = true;
        } elseif ($rSettings['automatic_backups'] == 'daily' && $rAdminSettings['automatic_backups_check'] < (time() - 86400)) {
            $bDoBackup = true;
        } elseif ($rSettings['automatic_backups'] == 'weekly' && $rAdminSettings['automatic_backups_check'] < (time() - 604800)) {
            $bDoBackup = true;
        } elseif ($rSettings['automatic_backups'] == 'monthly' && $rAdminSettings['automatic_backups_check'] < (time() - 2592000)) {
            $bDoBackup = true;
        }
    }
    if ($bDoBackup) {
        ...
        if (file_exists($rFilename)) {
            ...
            if ((count($rBackups) > intval($rAdminSettings["backups_to_keep"])) && (intval($rAdminSettings["backups_to_keep"]) > 0)) {
                ...
                foreach ($rDelete as $rItem) {
                    ipTV_lib::unlink_file(MAIN_DIR . "backups/" . $rItem["filename"]);
                }
            }
        }
    }
