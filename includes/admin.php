...
function writeAdminSettings() {
    global $rAdminSettings, $ipTV_db_admin;
    foreach ($rAdminSettings as $rKey => $rValue) {
        if (strlen($rKey) > 0) {
            $ipTV_db_admin->query("REPLACE INTO `admin_settings`(`type`, `value`) VALUES('" . $rKey . "', '" . $rValue . "');");
        }
    }
}
...
function updateTMDbCategories() {
    global $ipTV_db_admin, $rAdminSettings, $rSettings;
    include "tmdb.php";
...
function updateSeries($rID) {
    global $ipTV_db_admin, $rSettings, $rAdminSettings;
