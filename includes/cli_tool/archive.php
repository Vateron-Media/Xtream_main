...
if (@$argc) {
    if ($argc == 2) {
        ...
        if (0 >= $ipTV_db->num_rows()) {
            ...
            $fp = @fopen("http://127.0.0.1:" . CoreUtilities::$Servers[SERVER_ID]["http_broadcast_port"] . "/streaming/admin_live.php?password=" . CoreUtilities::$settings["live_streaming_pass"] . "&stream=" . $stream_id . "&extension=ts", "r");
            if ($fp) {
